...
#define VARTYPE_STRING 0x06
#define VARTYPE_VECTOR3 0x08

// --- ps2 will not compile without knowing about strncpy
#ifdef PS2
...
typedef union
{
	...
	int ival;	 // int
	int bval;	 // bool
	char strval[VARTYPE_STRING_SIZE];
} VAR_VAL;

// utilites
inline void SetVARVAL(VAR_VAL &var, int value) { var.ival = value; }
inline void SetVARVAL(VAR_VAL &var, bool value) { var.bval = value; }
inline void SetVARVAL(VAR_VAL &var, float value) { var.rval = value; }
inline void SetVARVAL(VAR_VAL &var, const char *value) 
	{ strncpy(var.strval, value, sizeof(var.strval) - 1); var.strval[sizeof(var.strval) - 1] = '\0'; }

inline int GetVARVALInt(const VAR_VAL &var) { return var.ival; }
inline bool GetVARVALBool(const VAR_VAL &var) { return (var.bval != 0); }
inline float GetVARVALFloat(const VAR_VAL &var) 	{ return var.rval; }
inline void GetVARVALString(const VAR_VAL &var, char *str, int strSize) 
	{ strncpy(str, var.strval, strSize - 1); str[strSize - 1] = '\0'; }
...
typedef struct
{
	...
	int		dataIndex;
	VAR_VAL	data;
	char		pad[PKT_SIZE - sizeof(VAR_VAL) - 20];
} TABLE_DATA;
