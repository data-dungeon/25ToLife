...
//-----------------------------------------------------------------------------
HRESULT CImage::Load( const VOID* pData, DWORD cbData, int x )
{
    HRESULT hr = S_OK;
...
//-----------------------------------------------------------------------------
HRESULT CImage::Load( char* pBuffer, int BufferSize )
{
//PAB    HANDLE hFile = CreateFileA( strFilename, GENERIC_READ, FILE_SHARE_READ, NULL, 
//PAB                              OPEN_EXISTING, FILE_FLAG_RANDOM_ACCESS, NULL );
//PAB    if( INVALID_HANDLE_VALUE == hFile )
//PAB        return E_FAIL;

    // Allocate memory
//PAB    DWORD dwFileSize = GetFileSize( hFile, NULL );
//PAB    VOID* pFileData  = malloc( dwFileSize );

    // Read it the file
//PAB    DWORD dwRead;
//PAB    ReadFile( hFile, (VOID*)pFileData, dwFileSize, &dwRead, NULL );

    HRESULT hr = Load( pBuffer, BufferSize, 0 );

    // Free stuff
//PAB    CloseHandle( hFile );
//PAB    free( pFileData );
...
HRESULT LoadImageFromImage( CImage* pDestImage, CImage* pSrcImage, 
                            DWORD dwFilter, D3DCOLOR ColorKey )
{
    RECT        srcRect;
    RECT        destRect;
...
HRESULT LoadVolumeFromVolume( CVolume* pDestVolume, CVolume* pSrcVolume, 
                              DWORD dwFilter, D3DCOLOR ColorKey )
{
    D3DBOX      srcBox;
    D3DBOX      destBox;
