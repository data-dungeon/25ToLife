...
void DXStaticMesh::activateSingleVertexStream( unsigned int stream, DXStreamRenderInfo & info )
{
	...
	if( stream != m_ActiveVertexStream )
	{
		p_ActiveVB = loadDXVertexStream( * a_SingleStreams[stream].p_Stream );
		ASSERT( p_ActiveVB );
...
void DXStaticMesh::applyActiveStreams( void )
{
	ASSERT( p_ActiveVB  &&  p_ActiveIB );

	setStreamSource( 0, *p_ActiveVB, 0, a_SingleStreams[m_ActiveVertexStream].p_Stream->m_VertexStride );
	setVertexDeclaration( a_SingleStreams[m_ActiveVertexStream].m_Desc.p_Descriptor );

	setIndices( *p_ActiveIB );
}

void DXStaticMesh::testDIP( const DXDIPData & DIPData )
{
	unsigned int prim = 0;
	unsigned int vertex = 0;

	unsigned short * pIndices = (unsigned short *) a_IndexStreams[m_ActiveIndexStream].p_IndexData;

	DXVertexStream * pVertexStream = a_SingleStreams[m_ActiveVertexStream].p_Stream;
	unsigned int colorOffset = (a_SingleStreams[m_ActiveVertexStream].m_Desc.m_StreamFlags & STFLAG_NORMAL) ? 7 : 4;

	unsigned int *	  pVertexData = (unsigned int *) pVertexStream->p_StreamData;

	pIndices += DIPData.d_IBBase;

	while( prim < DIPData.d_DIPPrimitiveCount )
	{
		unsigned int vertexOffset = *pIndices * pVertexStream->m_VertexStride;

		unsigned int * pVertex = (unsigned int *) ((unsigned int) pVertexData + vertexOffset);

		unsigned int	matrixSlot = (unsigned int) ((float *) pVertex)[3];
		assert( matrixSlot < 16 );

		unsigned char * pRGBData = (unsigned char *) (pVertex + colorOffset);

		unsigned int	matrixIdx = (unsigned int) pRGBData[3];

		unsigned int	loadedMatrix = p_CachedMatrices[matrixSlot];

		if( loadedMatrix != matrixIdx )
		{
			unsigned int stripIdx = (unsigned int) pRGBData[0];
			unsigned int mlistIdx = (unsigned int) pRGBData[1];
