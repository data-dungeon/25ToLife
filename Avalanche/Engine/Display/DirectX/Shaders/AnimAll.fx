...
LightPoint			mPointLights[3]				: register(c9);		//  9 registers	@	c9
LightDirectional	mDirectionalLights[3]		: register(c18);	//  6 registers	@	c18

float4				mWorldEye					: register(c24);	//  1 register	@	c24
LightSpecular		mSpecularLight				: register(c25);	//  2 registers @	c25

float3				mSkyVec						: register(c27);	//  1 register  @	c27
float4				mAmbientSky					: register(c28);	//  1 register	@	c28
float4				mAmbientGround				: register(c29);	//  1 register	@	c29
...
float3 applySpecularLight( LightSpecular Light, float3 worldEye, float3 worldVertex, float3 worldNormal )
{
    float	power = Light.Pos.w;
    float3	eyeVec = normalize( worldEye - worldVertex );
    float3	lightVec = normalize( Light.Pos - worldVertex );
    
	float3	halfAngle = (eyeVec + lightVec) * cHalf;
	
	float3	light = clamp( dot( worldNormal, halfAngle), 0.0f, 1.0f );
	
	return Light.Color * pow( light, power );
}
...
VS_OUTPUT AnimAll( VS_INPUT i )
{
	...
	if( mSpecularLight.Pos.w > 0.0f )
		o.Specular.rgb = applySpecularLight( mSpecularLight, mWorldEye, worldPos, worldNormal );
