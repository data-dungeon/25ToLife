...
class CLASS_EXPORT DBWriter
{
	...
	void 				Init( void);
	void 				WriteChunkHdr( unsigned short usType, t_sflagbits dblFlags);
...
public:
	...
	bool				DBStart( t_lflagbits dbStartFlags);
	bool				DBEnd( void);
	bool				ChunkStart( unsigned short usType, t_sflagbits dbChunkFlags);
	bool				ChunkStart( unsigned short usType, t_sflagbits dbChunkFlags, const char *pcName);
	bool				ChunkEnd( void);
