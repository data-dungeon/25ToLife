...
{
	if (theObjectAttributes.bDialogUp == true && theObjectAttributes.bRefreshWindowEnableScript)
	{
		// request for selection
		SendMessage(theObjectAttributes.hDlg,WM_SELCHANGE,1,0);
	}
}
...
static void NotifyBeforeOpen(void * data)
{
	if (theObjectAttributes.bDialogUp == true && theObjectAttributes.bRefreshWindowEnableScript)
	{
		...
		SendMessage(theObjectAttributes.hDlg,WM_SELCHANGE,0,0);
	}
}
...
// default constructor
{
   ...
   bRefreshWindowEnableScript = true;
...
// execute from a MEL command
// see undoredostate.h for command details
MStatus ObjectAttributes::ExecuteCommand(const MArgDatabase& argDatabase, MStringArray& resultStrings, UndoRedoState &undoRedoState)
{
	MStatus status;
	RefreshAttributes(); // this should be done most of the time anyway

	// query mode
	if (argDatabase.isFlagSet(kQueryFlag))
	{
		if (argDatabase.isFlagSet(kVersionFlag))
		{
			resultStrings.append(MString(g_pBuildVersion));
			return MS::kSuccess;
		}
		else if (argDatabase.isFlagSet(kAttributeConfigPathFlag))
		{
			...
			if (configIndex != -1)
			{
				...
				resultStrings.append(MString(fileString));
				return MS::kSuccess;
			}
			else
			{
				MGlobal::displayError(MString("Config file not set"));
				return MS::kFailure;
			}
		}
		else if (argDatabase.isFlagSet(kAttributeConfigNameFlag))
		{
			int configIndex = m_pConfigFilesManager->GetCurrentConfigIndex();
			if (configIndex != -1)
			{
				TupString nameString = m_pConfigFilesManager->GetName(configIndex);
				resultStrings.append(MString(nameString));
				return MS::kSuccess;
			}
			else
			{
		...
		{
			m_pController->HandleQueryCommand(argDatabase,resultStrings,undoRedoState);
		}
	}
	// set mode
	else if (argDatabase.isFlagSet(kSetFlag))
	{
		m_pController->HandleSetCommand(argDatabase,resultStrings,undoRedoState);
	}
	// purge mode
	else if (argDatabase.isFlagSet(kPurgeFlag))
	{
		m_pController->HandlePurgeCommand(argDatabase,resultStrings,undoRedoState);
	}
	// find mode
	else if (argDatabase.isFlagSet(kFindFlag))
	{
		m_pController->HandleFindCommand(argDatabase,resultStrings,undoRedoState);
	}
	// remap mode
	else if (argDatabase.isFlagSet(kRemapFlag))
	{
		m_pController->HandleRemapCommand(argDatabase,resultStrings,undoRedoState);
	}
	// apply defaults mode
	else if (argDatabase.isFlagSet(kApplyDefaultsFlag))
	{
		m_pController->HandleApplyDefaultValuesCommand(argDatabase,resultStrings,undoRedoState);
	}
	// remove unused
	else if (argDatabase.isFlagSet(kPurgeUnusedFlag))
	{
		m_pController->HandlePurgeUnusedCommand(argDatabase,resultStrings,undoRedoState);
	}
   // refresh dialog
   else if (argDatabase.isFlagSet(kRefreshDialogFlag))
   {
      m_pController->HandleRefreshDialogCommand(argDatabase,resultStrings,undoRedoState);
   }
...
{
	if (bDialogUp == true && bRefreshWindowEnable == true && bRefreshWindowEnableScript == true)
	{
		// request for selection
