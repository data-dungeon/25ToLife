...
enum {EXPORT_ACTIONCACHE, IMPORT_ACTIONCACHE, EXPORT_ACTIONCACHECONTAINER, IMPORT_ACTIONCACHECONTAINER, EXPORT_MULTICHORACTION };
...
extern "C" __declspec(dllexport) BOOL HxtOnAddCommandMenu(HTreeObject *htreeobject, UINT index, String &menuname, MenuCategory &mc, BOOL &disabled)
{
   ...
   case IMPORT_ACTIONCACHECONTAINER:
      ...
      return TRUE;
   case EXPORT_MULTICHORACTION: 
      if (((HAnimObject *)htreeobject)->IsCache()) // Actually a ModelCache, not a Model
         return FALSE;
      mc = MC_EXPORT;
      string.LoadString(IDS_MULTIACTION);
      menuname = (LPCTSTR)string;
      return TRUE;
   }
   return FALSE;
...
extern "C" __declspec(dllexport) BOOL HxtOnCommand(HTreeObject *htreeobject, UINT index)
{
   ...
   case EXPORT_MULTICHORACTION:
      return ((CHxtApp *)AfxGetApp())->OnExportMultiAction((HModel *)htreeobject);
   }
   return FALSE;
}
