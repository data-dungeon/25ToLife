...
class DXMeshBase
{
	...
	static DynamicIB *	p_UserIB;

	static void *			p_LastIB;

	static CXBPackedResource* m_Resources;
...
protected:
	...
	IDirect3DVertexBuffer8 * loadDXVertexStream( DXVertexStream & DXStream, bool bNeedZeroBase = false);

	void 					initDXIndexStream( IndexDataRecord & IBData, char * pBase, DXIndexStream & DXData );
	IDirect3DIndexBuffer8 *	loadDXIndexStream( DXIndexStream & DXStream );	
...
public:
	...
	static void				setActiveResources( CXBPackedResource * pResources );
