...
u32 DBWriter::FinishChunk(void)
{
	...
	for (int linkIndex=0;linkIndex<numFromLinks;linkIndex++)
	{
		...
		if (linkFrom.m_linkToHandle!=NULL_HANDLE)
		{
			...
			{
				throw(PluginException(ERROR_FATAL,"FinishChunk link %s->%s Write Failed\n  %s",
	...
	if (bError)
		throw(PluginException(ERROR_FATAL,"Error(s) in FinishChunk\n"));
...
void DBWriter::AlignFileQuadWord(void)
{
	...
	if (ulSize != 16)
	{
		...
		{
			throw(PluginException(ERROR_FATAL,"AlignFileQuadWord - WriteData failed\n  %s",(const char *)(e)));
		}
	}
}
...
void DBWriter::AlignFile32Byte(u8 padChar)
{
	...
	if (ulSize != 32)
	{
		...
		{
			throw(PluginException(ERROR_FATAL,"AlignFile32Byte - WriteData failed\n  %s",(const char *)(e)));
		}
	}
}
