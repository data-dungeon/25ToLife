...
IF /i [%1]==[] SET /p sandbox_choice=Which sandbox would you like to get services log from?
...
del "SandboxBackup_%SANDBOX_NAME%\*%SANDBOX_LOG_FILE%"
del "SandboxBackup_%SANDBOX_NAME%\Quazal Rendez-Vous.log"
del "SandboxBackup_%SANDBOX_NAME%\QuazalAuthentication.log"
echo lcd SandboxBackup_%SANDBOX_NAME% >>SandboxCommands.tmp
echo get "%SANDBOX_NAME%/Logs/*%SANDBOX_LOG_FILE%" >>SandboxCommands.tmp
echo get "%SANDBOX_NAME%/Logs/Quazal Rendez-Vous.log" >>SandboxCommands.tmp
echo get "%SANDBOX_NAME%/Logs/QuazalAuthentication.log" >>SandboxCommands.tmp
