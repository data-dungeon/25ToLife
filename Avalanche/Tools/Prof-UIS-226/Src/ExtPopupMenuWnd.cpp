...
//		pToolBar->PostMessage( WM_CANCELMODE );
		...
		)
	{
		if( pCmdItem != NULL )
		{
			...
			if( (m_dwTrackFlags&TPMX_NO_WM_COMMAND)==0 )
...
bool bTopLevel = m_bTopLevel;
	...
	if( ! bCusomizeMode )
	{
//		::PostMessage( hWndOwn, WM_CANCELMODE, 0L, 0L );
//		::PostMessage( hWndOwn, WM_CLOSE, 0L, 0L );
		_ItemFocusDelay();
		DestroyWindow();
	} // if( ! bCusomizeMode )
...
HWND hWndFocus = ::GetFocus();
	if(		hWndFocus != NULL
		...
		&&	(	nChar != VK_RETURN
			||	m_nCurIndex < 0
			||	(! _GetItemRef(m_nCurIndex).IsInplaceEdit() )
			)
		)
	{
		bool bAlt =
...
int nItemIndex = 0, nItemCount = m_items_all.GetSize();
	for( ; nItemIndex < nItemCount; nItemIndex++ )
	{
		MENUITEMDATA & _mii = m_items_all[ nItemIndex ];
...
bool bFullViewMode =
		(	m_bExpandWasPressed
		||	( (m_dwTrackFlags&TPMX_NO_HIDE_RARELY) != 0 )
		) ? true : false;
...
int nDisplayedCount = 0;
	for( nItemIndex = 0; nItemIndex < nItemCount; nItemIndex++ )
	{
		...
		if( _mii.IsDisplayed() )
		{
			...
			int nWidth = _mii.GetMeasuredWidth();
			if( m_sizeFullItems.cx < nWidth )
				m_sizeFullItems.cx = nWidth;
		} // if( _mii.IsDisplayed() )
		else
		{
			ASSERT( ! m_bExpandWasPressed );
	...
	{ // remove last separator
		...
		MENUITEMDATA & _mii = m_items_all[ nSeparatorIndex ];
		ASSERT( _mii.IsSeparator() );
		ASSERT( _mii.IsDisplayed() );
		_mii.SetDisplayed( false );
		int nMeasuredHeight = _mii.GetMeasuredHeight();
		m_sizeFullItems.cy -= nMeasuredHeight;
	} // if( nSeparatorIndex >= 0 )
...
CExtPopupColorMenuWnd::COLORREF_TABLE_ENTRY::COLORREF_TABLE_ENTRY(
	UINT nStrTblResID,
	COLORREF clrValue,
	__EXT_MFC_SAFE_LPCTSTR sColorNameEnglish
	)
	: m_clrValue( clrValue )
	, m_sColorNameEnglish( sColorNameEnglish )
#if (defined _AFXDLL && !defined __STATPROFUIS_WITH_DLLMFC__)
...
__EXT_MFC_SAFE_LPCTSTR CExtPopupColorMenuWnd::COLORREF_TABLE_ENTRY::GetLocalColorName() const
{
	__PROF_UIS_MANAGE_STATE;
#if (defined _AFXDLL && !defined __STATPROFUIS_WITH_DLLMFC__)
	if( ! m_sColorNameLocal.IsEmpty() )
		return m_sColorNameLocal;
CExtLocalResourceHelper _LRH;
	g_ResourceManager->LoadString( m_sColorNameLocal, m_nStrTblResID );
	if( ! m_sColorNameLocal.IsEmpty() )
		return m_sColorNameLocal;
	return m_sColorNameEnglish;
#else // if (defined _AFXDLL && !defined __STATPROFUIS_WITH_DLLMFC__)
	return m_sColorNameEnglish;
#endif // else from (defined _AFXDLL && !defined __STATPROFUIS_WITH_DLLMFC__)
}

// table captured from color picker
// control source by Chris Maunder
const CExtPopupColorMenuWnd::COLORREF_TABLE_ENTRY
	CExtPopupColorMenuWnd::g_colors[40] =
{
...
int nColorIdxCurr = _ColorItemHitTest(point);
	...
	if( m_nColorIdxCurr >= 0 )
	{
		...
		{
			CExtSafeString sTipText =
				LPCTSTR( g_colors[m_nColorIdxCurr].GetLocalColorName() );
			if( !sTipText.IsEmpty()
				&& GetSite().GetCapture() == this
				&& ( GetSite().GetAnimated() == NULL )
				)
...
COLORREF clr = RGB(0,0,0);
	...
	else
	{
		if( m_nColorIdxCurr < 0 )
			return;
		clr = g_colors[m_nColorIdxCurr].m_clrValue;
	}
	if( bFinal && m_pClrValueFinal != NULL )
...
int CExtPopupColorMenuWnd::_FindCellByColorRef(COLORREF clr)
{
	...
	for( int nIdx = 0; nIdx < __NCLR_COUNT; nIdx++ )
	{
		if( g_colors[nIdx].m_clrValue == clr )
			return nIdx;
	}
	return -1;
}
