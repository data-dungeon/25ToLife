...
//------------------------------------------------------------------------
// MapAttrController::MapAttrController
...
//------------------------------------------------------------------------
// MapAttrController::GetCurrAttribController
...
//------------------------------------------------------------------------
CurrAttribController* MapAttrController::GetCurrAttribController()
{
	return m_pCurrAttribController;
}

//------------------------------------------------------------------------
// MapAttrController::GetUIController
...
//------------------------------------------------------------------------
UIController* MapAttrController::GetUIController()
{
	return m_pUIController;
}


bool MapAttrController::LoadConfigFile(ConfigFilesManager &configFilesManager)
...
//------------------------------------------------------------------------
// MapAttrController::LoadCurrAttribData
...
//------------------------------------------------------------------------
bool MapAttrController::LoadCurrAttribData()
{
	...
	{	 
		...
		else if (type == MFn::kTransform || type == MFn::kMesh)
		{
			MDagPath objectPath;
			MObject component;
			...
			if (objectPath.hasFn(MFn::kMesh))
			{
				...
				CurrAttribController::GetFileTextures(objectPath,component,fileTextures);
				int numFileTextures = fileTextures.length();
				for (int fileTextureIndex=0;fileTextureIndex<numFileTextures;fileTextureIndex++)
				{
					MObject fileTextureObject = fileTextures[fileTextureIndex];

					int objectIndex = m_pCurrAttribController->GetIndex(fileTextureObject);
					// if this one is not in the controller already
					if (objectIndex==-1)
					{
						CurrAttribData* pNewAttribData = new CurrAttribData();
						...
						// Set the object pointer:
						objectIndex = m_pCurrAttribController->AddMayaObject(fileTextureObject);
						
						TupString strFullData;
						CurrAttribController::GetNote(fileTextureObject,strFullData);
...
//------------------------------------------------------------------------
// MapAttrController::LoadCurrAttribDataScene
...
//------------------------------------------------------------------------
bool MapAttrController::LoadCurrAttribDataScene()
{
	...
	for ( ; !depIterator.isDone(); depIterator.next() )
	{
		...
		m_pCurrAttribController->LoadCurrAttribDataNode(depIterator.item());
	}
	return bFoundNode;
}

//------------------------------------------------------------------------
// MapAttrController::SetUpUIDisplay
...
// Output: True if successful, false otherwise.
//------------------------------------------------------------------------
bool MapAttrController::SetUpUIDisplay(HWND hParentDlg, HINSTANCE hInstance,  HWND hToolTip,
...
//------------------------------------------------------------------------
// MapAttrController::DestroyCurrAttribData
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::DestroyCurrAttribData()
{
	if (m_pCurrAttribController != NULL)
	{
		m_pCurrAttribController->RemoveAll();
...
//------------------------------------------------------------------------
// MapAttrController::DestroyUIDisplay
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::DestroyUIDisplay()
{
	if (m_pUIController != NULL)
	{
		m_pUIController->RemoveToolTips(m_hParentDlg,m_hInstance,m_hToolTip);
...
//------------------------------------------------------------------------
// MapAttrController::SaveNonexclusiveKeys
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::SaveNonexclusiveKeys()
{
	if (m_pExclusiveController != NULL)
...
//		   returned.
//------------------------------------------------------------------------
int MapAttrController::GetTotalY()
{
	if (m_pUIController != NULL)
...
//------------------------------------------------------------------------
bool MapAttrController::OkToCopy(void) const
{
...
//------------------------------------------------------------------------
bool MapAttrController::OkToPaste(void) const
{
	// only able to copy into copy buffer if only one face selected
	if (m_pCurrAttribController != NULL)
	{
...
//------------------------------------------------------------------------
// MapAttrController::CopySelection
...
// Output: True if we are able to paste from the copy buffer
//------------------------------------------------------------------------
bool MapAttrController::CopySelection(void)
{
	bool bResult = false;
...
//------------------------------------------------------------------------
// MapAttrController::PasteAttribsToSelSet
...
//------------------------------------------------------------------------
bool MapAttrController::PasteAttribsToSelSet(bool bReplace)
{
	bool bResult = false;
	...
	if (m_pCurrAttribController != NULL)
	{
		...
		if (nNumObjects && m_pCopiedKeyValPairs)
		{
			...
			for (int nIndex = 0; nIndex < nNumObjects; nIndex++)
			{
				...
				TupString strFullData;
				CurrAttribController::GetNote(mObject,strFullData);
				TupString strAttribData = pDestAttribData->CurrObjDataToStr();
...
//------------------------------------------------------------------------
// MapAttrController::HandleWinMessage
...
//------------------------------------------------------------------------
bool MapAttrController::HandleWinMessage(HWND hParent, UINT message, WPARAM wParam, LPARAM lParam)
{
	bool bResult = false;
			...
			if (nFlags & UIDisplayBase::WIN_MESSAGE_FLAG_EXCLUSIVE)
			{
				...
				if (m_pCurrAttribController != NULL)
				{
					...
					if (nFlags & UIDisplayBase::WIN_MESSAGE_FLAG_PURGE_KEYS)
					{
						...
						{
							if (m_pCurrAttribController->RemoveKeyInstances(strKeyname, strValue, vectKeysToPurge[nLoop]))
							{
								bUpdateData = true;
							}
						}
...
//------------------------------------------------------------------------
// MapAttrController::StripAllButRawData
...
//		   otherwise.
//------------------------------------------------------------------------
bool MapAttrController::StripAllButRawData(TupString& strToFix)
{
...
//------------------------------------------------------------------------
// MapAttrController::AddRawDataToString
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::AddRawDataToString(TupString& strToAddTo,const TupString &strToInsert)
{
	// attempt to gracefully get to just carriage returns
	strToAddTo.Replace("\r\n","\r");
	strToAddTo.Replace("\n\r","\r");
...
//------------------------------------------------------------------------
// MapAttrController::InitKeyValPanel
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::InitKeyValPanel(ICustomRollupWindow* pCustomRollupWindow, HINSTANCE hInstance, LPARAM lParam)
{
	if (m_pKeyValComm == NULL)
	{
...
//------------------------------------------------------------------------
// MapAttrController::IdCancelMessage
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::IdCancelMessage()
{
	if (m_pKeyValComm != NULL)
...
//------------------------------------------------------------------------
// MapAttrController::SetApplyBttnStates
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::SetApplyBttnStates(bool bApplySelEnabled, bool bApplySceneEnabled)
{
	if (m_pKeyValComm != NULL)
...
//------------------------------------------------------------------------
// MapAttrController::LoadKeyValPanelTree
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::LoadKeyValPanelTree()
{
	if (m_pKeyValComm != NULL)
...
//------------------------------------------------------------------------
// MapAttrController::HandleApplySelect
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::HandleApplySelect(Keeper<KeyInfo>& keyKeeper, bool bApplyToAll)
{
	...
	// Tell the MapAttrController to get the current attribute data:
	if (m_pCurrAttribController != NULL)
	{
...
//------------------------------------------------------------------------
// MapAttrController::ObjectMatchesCheckedItems
...
//------------------------------------------------------------------------
bool MapAttrController::ObjectMatchesCheckedItems(CurrAttribData* pCurrAttribData, Keeper<KeyInfo>& checkedKeyKeeper)
{
...
//------------------------------------------------------------------------
// MapAttrController::BuildObjIndices
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::BuildObjIndices(TupString strKey, TupString strVal,
										TupArray<short>& vectObjIndices)
{
...
//------------------------------------------------------------------------
// MapAttrController::HandleApplyApply
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::HandleApplyApply(Keeper<KeyInfo>& keyKeeper, bool bApplyToAll)
{
...
//------------------------------------------------------------------------
// MapAttrController::HandleApplyPurge
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::HandleApplyPurge(Keeper<KeyInfo>& keyKeeper, bool bApplyToAll)
{
...
//------------------------------------------------------------------------
// MapAttrController::HandleApplyRemap
...
// Output: None.
//------------------------------------------------------------------------
void MapAttrController::HandleApplyRemap(Keeper<KeyInfo>& keyKeeper, bool bApplyToAll)
{
