 ...
 * 
 * *****************  Version 12  *****************
 * User: Adam Clayton Date: 11/14/02   Time: 12:54p
 * Updated in $/Avalanche/tools/TupperAttrib
 * formatting fixes
 * 
 * *****************  Version 11  *****************
 * User: Adam Clayton Date: 11/13/02   Time: 4:18p
 * Updated in $/Avalanche/tools/TupperAttrib
 * fixed build string
 * 
 * *****************  Version 10  *****************
 * User: Nmartin      Date: 8/30/02    Time: 9:07a
 * Updated in $/Avalanche/tools/TupperAttrib
 * change over to using models for inflow/outflow streams, and
 * TupperSttrib fix for checkboxes
 * 
 * *****************  Version 9  *****************
 ...
 * Updated in $/Avalanche/tools/TupperAttrib
 * Handle NULL pointer in constructor
 * 
 * *****************  Version 5  *****************
...
// Output: None
//-----------------------------------------------------------------------------
TupKeyValue::TupKeyValue(const char *pBuffer,const char *pBlockStart,const char *pBlockEnd)
{
...
bool TupKeyValue::StripAllButRawData(TupString& strToFix,const char *pBlockStart,const char *pBlockEnd)
{
...
void TupKeyValue::StripStringWhiteSpace(TupString &strToFix)
{
	strToFix.TrimLeft();
	strToFix.TrimRight();
}

bool TupKeyValue::GetBeforeBlockStart(TupString& strToFix,const char *pBlockStart)
{
	assert(pBlockStart);
	// Find and remove the begin block indicator:
	int beginIndex = strToFix.Find(pBlockStart);
...
bool TupKeyValue::GetAfterBlockEnd(TupString& strToFix,const char *pBlockEnd)
{
	assert(pBlockEnd);
	// Find and remove the begin block indicator:
	int endIndex = strToFix.Find(pBlockEnd);
...
//-----------------------------------------------------------------------------
bool TupKeyValue::RemoveCharsFromStr(TupString& strToFix, char chToRemove)
{
...
//-----------------------------------------------------------------------------
int TupKeyValue::KeyExistsAt(const char *pKey) const
{
	int nIndexAt = -1;

	// Cycle through all the keys
	int nNumKeys = m_keyValuePairArray.GetSize();
	for (int nIndex = 0; nIndex < nNumKeys; nIndex++)
	{
		if (m_keyValuePairArray[nIndex].m_strKey == pKey)
			nIndexAt = nIndex;
	}
	return nIndexAt;
}
...
// Output: Index of where keyname exists at.  -1 if it doesn't exist.
//-----------------------------------------------------------------------------
int TupKeyValue::KeyExistsAt(const TupString &key) const
{
	int nIndexAt = -1;
	...
	for (int nIndex = 0; nIndex < nNumKeys; nIndex++)
	{
		if (m_keyValuePairArray[nIndex].m_strKey == key)
			nIndexAt = nIndex;
	}
	return nIndexAt;
}
...
//-----------------------------------------------------------------------------
const char *TupKeyValue::GetKeyAsString(int nIndex) const
{
...
//-----------------------------------------------------------------------------
const char *TupKeyValue::GetValueAsString(int nIndex) const
{
...
//-----------------------------------------------------------------------------
bool TupKeyValue::GetValueAsBool(int nIndex) const
{
...
//-----------------------------------------------------------------------------
int TupKeyValue::GetValueAsInt(int nIndex) const
{
	if (stricmp((const char *)m_keyValuePairArray[nIndex].m_strValue, "true") == 0)
		return(1);
...
//-----------------------------------------------------------------------------
float TupKeyValue::GetValueAsFloat(int nIndex) const
{
...
//-----------------------------------------------------------------------------
TupAttribColor TupKeyValue::GetValueAsColor(int nIndex) const
{
...
//-----------------------------------------------------------------------------
int TupKeyValue::GetNumAttribs() const
{
	return m_keyValuePairArray.GetSize();
}

//-----------------------------------------------------------------------------
// TupKeyValue::AddKeyValue
// This method will return the number of attribs in this object.
//
// Input: 
...
//-----------------------------------------------------------------------------
int TupKeyValue::AddKeyValue(const char *pKey,const char *pValue,bool bReplace)
{
...
// Output: Int representing the position within the array for the key-value
//-----------------------------------------------------------------------------
int TupKeyValue::AddKeyValue(const TupString &key,const TupString &value,bool bReplace)
{
...
bool TupKeyValue::MergeKeyValues(const TupKeyValue &keysToAdd,bool bReplace)
{
	bool bChanged = false;
...
TupString TupKeyValue::BuildString(const char *pBuffer1,const char *pBuffer2,const char *pBlockStart,const char *pBlockEnd)
{
