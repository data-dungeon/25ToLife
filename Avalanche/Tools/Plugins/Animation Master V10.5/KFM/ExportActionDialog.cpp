...
CDialog( IDD_EXPORTACTION, CWnd::FromHandle(GetMainApplicationWnd()) )
{
   ...
   m_errortolerance = GetRegistryValue("KFM Plugin", "ReductionTolerance", 0.0f);

   m_exportframerange = GetRegistryValue("KFM Plugin", "ExportFrameRange", 0);
   m_beginframe = GetRegistryValue("KFM Plugin", "BeginFrame", 0);
   m_endframe = GetRegistryValue("KFM Plugin", "EndFrame", 0);
...
void CExportActionDialog::DoDataExchange(CDataExchange* pDX)
{
   ...
   DDX_Check(pDX, IDC_EXPORTFRAMERANGE, m_exportframerange);  
   DDX_Text(pDX, IDC_BEGINFRAME, m_beginframe);
   DDV_MinMaxInt(pDX, m_beginframe, 0,1000000);
   DDX_Text(pDX, IDC_ENDFRAME, m_endframe);
   DDV_MinMaxInt(pDX, m_endframe, 0,1000000);
...
void CExportActionDialog::OnOK()
{
   ...
   SetRegistryValue("KFM Plugin", "ExportFrameRange", m_exportframerange );
   SetRegistryValue("KFM Plugin", "BeginFrame", m_beginframe );
   SetRegistryValue("KFM Plugin", "EndFrame", m_endframe );
