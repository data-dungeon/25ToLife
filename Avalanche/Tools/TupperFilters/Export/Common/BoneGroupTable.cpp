		...
		for (TupperwareIterator iter = pBoneGroupNamesAgg->FindAtomByKey("Entry"); !iter.Finished(); iter.Next()) 
		{
			...
			if (!pItemAtom->GetName())
			{
				PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
					"BoneGroupNames - Entry specified without the name");
				m_numErrors++;
				continue;
			}
			...
			{
				PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
					"Invalid BoneGroupName %s - Use Entry ""NAME"" = VALUE",pItemAtom->GetName());
				m_numErrors++;
				continue;
			}
			...
			if (newIndex==oldIndex)
			{
				...
				{
					PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
						"BoneGroupsNames - BoneGroup for name %s needs to be between 0 and 31 currently set to %d",pItemAtom->GetName(),value);
					m_numErrors++;
					continue;
				}
			...
			{
				PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
					"Duplicate BoneGroupName found %s",pItemAtom->GetName());
				m_numErrors++;
			}
		}
	}
...
void BoneGroupTable::GetBoneGroupData(TupperwareAggregate *pTupRoot)
{
	...
	{
		PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
			"Cannot find BoneGroups within group table file");
		m_numErrors++;
		return;
	}
	...
	for (TupperwareIterator iter = pBoneGroupAgg->FindAll(); !iter.Finished(); iter.Next()) 
	{
		...
		{
			PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
				"BoneGroups - Invalid Bone %s - Use NAME [""MIRRORNAME""] = VALUE|""NAME""",pItemAtom->GetKey());
			m_numErrors++;
			continue;
		}
		...
		if (pItemScalar->GetDataType()==TupperwareScalar::DATA_INT)
		{
			...
			{
				PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
					"BoneGroups - BoneGroup for bone %s needs to be between 0 and 31 currently set to %d",pItemAtom->GetKey(),value);
				m_numErrors++;
				continue;
			}
		...
		else if (pItemScalar->GetDataType()==TupperwareScalar::DATA_STRING)
		{
			...
			{
				PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
					"BoneGroups - Bone %s cannot find %s in BoneGroupNames",pItemAtom->GetKey(),pItemScalar->GetAsString());
				m_numErrors++;
				continue;
			}
		...
		{
			PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
				"Duplicate Bone Found %s",pItemAtom->GetKey());
			m_numErrors++;
			continue;
		}
	}
	...
	for (int entryIndex = 0;entryIndex<numEntries;entryIndex++)
	{
		...
		if (entry.m_mirrorName.GetLength())
		{
			...
			{
				PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
					"Mirror Bone Not Found %s -> %s",(const char *)entry.m_name,(const char *)entry.m_mirrorName);
				m_numErrors++;
				continue;
			}
			...
			{
				PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
					"Mirrored Bones not in sync %s -> %s - %s -> %s",(const char *)entry.m_name,(const char *)entry.m_mirrorName,(const char *)mirrorEntry.m_name,(const char *)mirrorEntry.m_mirrorName);
				m_numErrors++;
				continue;
			}
...
{
	TupperwareAggregate *pInvalidModelChannelsAgg = pTupRoot->FindAggregateByKey("InvalidModelChannels");
	if (pInvalidModelChannelsAgg)
	{
		for (TupperwareIterator iter = pInvalidModelChannelsAgg->FindAtomByKey("Entry"); !iter.Finished(); iter.Next()) 
		{
			...
			{
				PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
					"Invalid Channel Use Entry ""NAME"" = VALUE");
				m_numErrors++;
				continue;
			}
			...
			{
				PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
					"Invalid Channel %s - Use Entry ""NAME"" = VALUE",pItemAtom->GetName());
				m_numErrors++;
				continue;
			}
			TupperwareScalar *pItemScalar = (TupperwareScalar *) pItemAtom;
			...
			{
				PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
					"Duplicate Invalid Model Channel found %s",pItemAtom->GetName());
				m_numErrors++;
			}
		}
	}
	...
	{
		PluginLog::Add(LogFlags::VERBOSE_LEVEL_MED|LogFlags::FONT_COLOR_ORANGE,
			"InvalidModelChannels not found.  Using defaults");
		int value1 = -1;
		int value2 = 31;
...
void BoneGroupTable::GetInvalidMotionChannelData(TupperwareAggregate *pTupRoot)
{
	...
	if (pInvalidMotionChannelsAgg)
	{
		for (TupperwareIterator iter = pInvalidMotionChannelsAgg->FindAtomByKey("Entry"); !iter.Finished(); iter.Next()) 
		{
			...
			{
				PluginLog::Add(LogFlags::VERBOSE_LEVEL_NONE|LogFlags::FONT_COLOR_DK_RED|LogFlags::FONT_STYLE_BOLD,
					"Duplicate Invalid Motion Channel found %s",pItemAtom->GetName());
				m_numErrors++;
			}
		}
	}
	...
	{
		PluginLog::Add(LogFlags::VERBOSE_LEVEL_MED|LogFlags::FONT_COLOR_ORANGE,
			"InValidMotionChannels not found.  Using defaults");
		int value = -1;
		m_invalidMotionChannelNames.Add(TupString("control"));
		m_invalidMotionChannelValues.Add(value);
		m_invalidMotionChannelNames.Add(TupString("shortcut"));
