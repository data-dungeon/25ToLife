...
#ifndef TUPPER3D_VERTEX_PAINT_MODIFIER_H
#define TUPPER3D_VERTEX_PAINT_MODIFIER_H

class TupModifier;

class TupVertexPaintModifier
{
	// Private Default constructor so no one can use it
	TupVertexPaintModifier(void);
...
public:
	enum VertexPaintModifierFlags
	{
		FLAGS_ENABLED			= 1<<0,
	};

	TupVertexPaintModifier(TupperwareAggregate *pObject);
	TupVertexPaintModifier(TupModifier &modifierBase);
...
private:
// add methods
	TupperwareScalar *AddIntScalar(int value,TupVertexPaintModifier::ChildrenAtoms index);

// get methods
	TupperwareScalar *GetIntScalar(int &value,TupVertexPaintModifier::ChildrenAtoms index);
};
