...
{
	CPerFaceDataMod *mod = (CPerFaceDataMod *)imod;
	Invalidate(PART_SELECT);

	if (!theHold.Holding())	theHold.Begin();
	if ( theHold.Holding())	theHold.Put(new CSelectRestoreObj(mod, this, SELECT_VERTEX));
...
void CModData::SetFaceSel(BitArray &set, IMeshSelect *imod, TimeValue t) 
{
	...
	if (!theHold.Holding())	theHold.Begin();
	if ( theHold.Holding())	theHold.Put(new CSelectRestoreObj(mod, this, SELECT_FACE));
...
void CModData::SetEdgeSel(BitArray &set, IMeshSelect *imod, TimeValue t) 
{
	...
	if (!theHold.Holding())	theHold.Begin();
	if ( theHold.Holding())	theHold.Put(new CSelectRestoreObj(mod, this, SELECT_EDGE));
