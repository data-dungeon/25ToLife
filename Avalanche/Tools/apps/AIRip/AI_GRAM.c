/* c:\mks\bin\yacce -l -S states.out -v -t -D ai_gram.h -o ai_gram.c .\ai_gram.y */
#ifdef YYTRACE
#define YYDEBUG 1
#else
#ifndef YYDEBUG
...
#if YYDEBUG
enum YY_Types { YY_t_NoneDefined, YY_t_pcString, YY_t_nValue, YY_t_fValue
};
#endif
#if YYDEBUG
...
long yyStates[] = {
0L, 44L, 55L, 69L, 91L, 135L, 149L, 163L, 177L, 188L, 202L, 216L, 227L, 
275L, 289L, 300L, 311L, 326L, 348L, 392L, 407L, 422L, 437L, 452L, 467L, 
482L, 504L, 548L, 571L
};
#define YYMAX_READ	62
#endif
static short yydef[] = {
...
static short yyact[] = {
	 ...
	 -18,  -25,  -11,  -10,  -12,  -13,  -14,  -15,  267,  266, 
	 265,  264,  263,  262,  261,  257,  -16,  -17,  260,  259, 
	 -22,  258,  -23,  258,  -24,  258,  -26,  261,  -27,  261,   -1
};
static short yypact[] = {

	   8,    8,   30,   18,   18,   18,   18,   29,   27,   25, 
	  23,   21,   18,   18,   18
};
static short yygo[] = {

	 -21,  -20,   -4,   -5,   -6,   -7,  -19,   13,   12,    6, 
	   5,    4,    3,   -3,   -2,  -28,  -29,    1,   -8,   -9,   -1
};
static short yypgo[] = {

	   0,    0,    0,   18,    6,    6,   19,   18,   18,   18, 
	  18,   18,   18,   16,   16,   16,   14,   14,   18,   13, 
	  13,    0
};
static short yyrlen[] = {
...
short yyrmap[] = {
	  ...
	  19,   20,   21,   15,   18,   17,   16,   14,   13,   12, 
	  11,   10,    9,    7,    6,    5,    3,    4,    8,    2, 
	   1,    0
};
short yysmap[] = {

	   0,   12,   13,   26,   25,   18,   17,   10,    9,    7, 
	   6,    5,    4,    3,    2,   14,   15,    1,   16,   27, 
	  28,   19,   20,   21,    8,   22,   23,   24,   11
};
int yyntoken = 15;
int yynvar = 7;
int yynstate = 29;
...
case YYr5: {	/* statement :  command CR_ */
iLineCount++;
} break;

case YYr6: {	/* statement :  label CR_ */
iLineCount++;
} break;

case YYr7: {	/* statement :  CR_ */
iLineCount++;
} break;

case YYr9: {	/* command :  DW TEXT_ */
ProcessDW ( yypvt[0].pcString);
} break;

case YYr10: {	/* command :  AI_TABLE TEXT_ */
ProcessAiTable ( yypvt[0].pcString);
} break;

case YYr11: {	/* command :  JUMP TEXT_ */
ProcessJump ( yypvt[0].pcString);
} break;

case YYr12: {	/* command :  MOVETO floatexp floatexp floatexp */
ProcessMoveto ( yypvt[-2].fValue, yypvt[-1].fValue, yypvt[0].fValue);
} break;

case YYr13: {	/* command :  ORIGIN floatexp floatexp floatexp */
ProcessOrigin ( yypvt[-2].fValue, yypvt[-1].fValue, yypvt[0].fValue);
} break;

case YYr14: {	/* command :  DELTA floatexp */
ProcessDelta ( yypvt[0].fValue);
} break;

case YYr15: {	/* command :  MOVETO floatexp floatexp floatexp */
ProcessMoveto ( yypvt[-2].fValue, yypvt[-1].fValue, yypvt[0].fValue);
} break;

case YYr16: {	/* label :  LABEL_ */
ProcessLabel ( yypvt[0].pcString);
} break;

case YYr17: {	/* floatexp :  FLOAT_ */
 yyval.fValue = yypvt[0].fValue; 
} break;

case YYr18: {	/* floatexp :  INTEGER_ */
 yyval.fValue = (float) yypvt[0].nValue; 
} break;
	case YYrACCEPT:
		YYACCEPT;
	case YYrERROR:
