...
MapAttributes theMapAttributes;
	...
	case WM_LBUTTONDBLCLK:
		{
			...
			if (theMapAttributes.HandleWinMessage(hwnd, uMsg, wParam, lParam) == true)
			{
				// We don't need to rebuild the UIDisplay for this message!
				// But we do need to load the key/val panel:
				theMapAttributes.LoadKeyValPanelTree();
			}
...
//
{
	switch (message)
	{
		case WM_INITDIALOG:
		{
			...
			theMapAttributes.m_hAttributesToolTip = CreateWindowEx(WS_EX_TOOLWINDOW | WS_EX_TOPMOST,
			...
			ICustomToolTip *pToolTip = ICustomToolTip::GetIToolTip(theMapAttributes.m_hAttributesToolTip);
			...
			else
			{
				...
				theMapAttributes.SetApplyBttnStates(false, false);
			}
		...
		case WM_PAINT:
		{
			...
			theMapAttributes.DrawUIDisplayLines(hdc);
			EndPaint( hDlg, &ps );
	    ...
	    case WM_COMMAND:
		{
			switch(HIWORD(wParam))
			{
				...
				case BN_CLICKED:
				{
					...
					if (theMapAttributes.HandleWinMessage(hDlg, message, wParam, lParam) == true)
					{
						// Set up the UI Display:
						theMapAttributes.SetUpUIDisplay(hDlg, hInstance,
							START_X, START_Y,
		...
		case WM_NOTIFY:
		{
			...
			switch (hdr->code)
			{
				...
				case ICustomToolTip::UDM_TOOLTIP_DISPLAY:
				{
					...
					theMapAttributes.HandleWinMessage(hDlg, message, wParam, lParam);
				}
				break;
			}
...
//		case CC_COLOR_CHANGE:
		...
		case WM_SETCURSOR:
		{
			if (NULL != theMapAttributes.m_hAttributesToolTip)
			{
		...
		case WM_SELCHANGE:
		{
			...
			else
			{
				...
				theMapAttributes.DestroyCurrAttribData();
				theMapAttributes.DestroyUIDisplay();

				// adjust the panel heights
				theMapAttributes.AdjustPanelHeights(true);
			}
...
{
	if (theMapAttributes.bDialogUp == true)
	{
		// request for selection
		SendMessage(theMapAttributes.hDlg,WM_SELCHANGE,1,0);
	}
}
...
static void NotifyBeforeOpen(void * data)
{
	if (theMapAttributes.bDialogUp == true)
	{
		...
		SendMessage(theMapAttributes.hDlg,WM_SELCHANGE,0,0);
	}
}
...
{
	if (!theMapAttributes.hDlg) // all ready done?
		return;

	// save off the position of the dialog
	RECT dialogRect;
	GetWindowRect(theMapAttributes.hDlg, &dialogRect);
	...
	ICustomRollupWindow* pRollupWindow = ICustomRollupWindow::GetICustomRollupWindow(GetDlgItem(theMapAttributes.hDlg, IDC_CUSTOM_ROLLUP));
	...
	theMapAttributes.SaveNonexclusiveKeys();

	// Now clean up:
	// Release any controls:
	if (theMapAttributes.m_hIconOneToMany)
		::DestroyIcon(theMapAttributes.m_hIconOneToMany);
	if (theMapAttributes.m_hIconOneToManyDisabled)
		::DestroyIcon(theMapAttributes.m_hIconOneToManyDisabled);
	if (theMapAttributes.m_hIconReplace)
		::DestroyIcon(theMapAttributes.m_hIconReplace);
	if (theMapAttributes.m_hIconReplaceDisabled)
		::DestroyIcon(theMapAttributes.m_hIconReplaceDisabled);
	if (theMapAttributes.m_hIconBrowse)
		::DestroyIcon(theMapAttributes.m_hIconBrowse);

	// Clean up things dealing with the keyval panel:
	theMapAttributes.IdCancelMessage();

	// Unregister the rollup callback:
	ICustomRollupWindow::ReleaseICustomRollupWindow(pRollupWindow);

	theMapAttributes.bDialogUp = false;

	EndDialog(theMapAttributes.hDlg, FALSE);
	theMapAttributes.DestroyUIDisplay();
	theMapAttributes.DestroyCurrAttribData();
	theMapAttributes.hDlg = NULL;
}
	...
	switch (msg)
	{
		case WM_INITDIALOG:
		{
				...
				(myRegistry.Get(DIALOG_RIGHT_VAL_NAME, &dwRight) == TRUE))
			{
				...
				MoveWindow(theMapAttributes.hDlg,
			...
			{
				WinUtil::CenterWindow(theMapAttributes.hDlg, GetParent(theMapAttributes.hDlg));
			}
			
			theMapAttributes.m_hOneToManyButton = GetDlgItem(hWnd, IDC_ONETOMANY);
			theMapAttributes.m_hReplaceButton = GetDlgItem(hWnd, IDC_REPLACE);
			theMapAttributes.m_hCopyButton = GetDlgItem(hWnd, IDC_COPY_BUTTON);
			theMapAttributes.m_hConfigList = GetDlgItem(hWnd, IDC_CFG_LIST);
			theMapAttributes.m_hBrowseButton = GetDlgItem(hWnd, IDC_BROWSE);
			
			// subclass the buttons to handle the killfocus problem
			SetWindowLongPtr(theMapAttributes.m_hOneToManyButton,GWLP_USERDATA,(LONG)SetWindowLongPtr(theMapAttributes.m_hOneToManyButton,GWL_WNDPROC,(LONG)PushButtonProc));
			SetWindowLongPtr(theMapAttributes.m_hReplaceButton,GWLP_USERDATA,(LONG)SetWindowLongPtr(theMapAttributes.m_hReplaceButton,GWL_WNDPROC,(LONG)PushButtonProc));
			SetWindowLongPtr(theMapAttributes.m_hCopyButton,GWLP_USERDATA,(LONG)SetWindowLongPtr(theMapAttributes.m_hCopyButton,GWL_WNDPROC,(LONG)PushButtonProc));
			SetWindowLongPtr(theMapAttributes.m_hBrowseButton,GWLP_USERDATA,(LONG)SetWindowLongPtr(theMapAttributes.m_hBrowseButton,GWL_WNDPROC,(LONG)PushButtonProc));
			
			theMapAttributes.m_hIconOneToMany = (HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IDI_ONETOMANY), IMAGE_ICON, 0, 0, 0);
			theMapAttributes.m_hIconOneToManyDisabled = (HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IDI_ONETOMANY_DISABLED), IMAGE_ICON, 0, 0, 0);
			theMapAttributes.m_hIconReplace = (HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IDI_REPLACE), IMAGE_ICON, 0, 0, 0);
			theMapAttributes.m_hIconReplaceDisabled = (HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IDI_REPLACE_DISABLED), IMAGE_ICON, 0, 0, 0);
			theMapAttributes.m_hIconCopy = (HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IDI_COPY), IMAGE_ICON, 0, 0, 0);
			theMapAttributes.m_hIconCopyDisabled = (HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IDI_COPY_DISABLED), IMAGE_ICON, 0, 0, 0);
			theMapAttributes.m_hIconBrowse = (HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IDI_BROWSE), IMAGE_ICON, 0, 0, 0);
			
			::SendMessage(theMapAttributes.m_hOneToManyButton, BM_SETIMAGE, IMAGE_ICON, (LPARAM)theMapAttributes.m_hIconOneToManyDisabled);
			::SendMessage(theMapAttributes.m_hReplaceButton, BM_SETIMAGE, IMAGE_ICON, (LPARAM)theMapAttributes.m_hIconReplaceDisabled);
			::SendMessage(theMapAttributes.m_hCopyButton, BM_SETIMAGE, IMAGE_ICON, (LPARAM)theMapAttributes.m_hIconCopyDisabled);
			::SendMessage(theMapAttributes.m_hBrowseButton, BM_SETIMAGE, IMAGE_ICON, (LPARAM)theMapAttributes.m_hIconBrowse);
			
			// set up tooltips
			theMapAttributes.m_hToolTip = CreateWindowEx(WS_EX_TOOLWINDOW | WS_EX_TOPMOST,
				CustomControls::GetWindowClassName(CustomControls::TOOLTIP_WINDOW_CLASS), NULL,
				WS_POPUP, 
				0,0,0,0,
				hWnd, NULL, hInstance,
			...
			ICustomToolTip *pToolTip = ICustomToolTip::GetIToolTip(theMapAttributes.m_hToolTip);
			assert(pToolTip);
			...
			pToolTip->AddTool(theMapAttributes.m_hBrowseButton,0);
			pToolTip->AddTool(theMapAttributes.m_hOneToManyButton,0);
			pToolTip->AddTool(theMapAttributes.m_hReplaceButton,0);
			pToolTip->AddTool(theMapAttributes.m_hCopyButton,0);
			pToolTip->AddTool(theMapAttributes.m_hConfigList,0);
			pToolTip->SetNotify(TRUE);
			pToolTip->SetNotify(theMapAttributes.hDlg);
			pToolTip->Activate(TRUE);

			ICustomToolTip::ReleaseIToolTip(pToolTip);

			::SendMessage(theMapAttributes.m_hConfigList,CB_RESETCONTENT,0,0L);
			ConfigFilesManager &configFilesManager = theMapAttributes.GetConfigFilesManager();
			...
			{
				int comboIndex = ::SendMessage(theMapAttributes.m_hConfigList,CB_ADDSTRING,0,(LPARAM)((const char *)configFilesManager.GetName(configFileIndex)));
				::SendMessage(theMapAttributes.m_hConfigList,CB_SETITEMDATA,comboIndex,configFileIndex);
				if (configFileIndex==configFilesManager.GetCurrentConfigIndex())
				{
					::SendMessage(theMapAttributes.m_hConfigList,CB_SETCURSEL,comboIndex,0L);
				}
			}
			...
			theMapAttributes.InitKeyValPanel(pRollupWindow, hInstance, (LPARAM)theMapAttributes.hDlg);
			...
			pRollupWindow->AppendRollup(hInstance, MAKEINTRESOURCE(IDD_ATTRIBUTES),
				AttributesRollupPageProc, attribBuf, (LPARAM)theMapAttributes.hDlg,
				0);
			...
			theMapAttributes.AdjustPanelHeights();
	    ...
	    case WM_COMMAND:
		{
			...
			else if (HIWORD(wParam)==BN_CLICKED)
			{
				switch(LOWORD(wParam))
				{
					case IDC_ONETOMANY:
					{
						theMapAttributes.PasteAttribsToSelSet(false);
						return TRUE;
					}
					case IDC_REPLACE:
					{
						theMapAttributes.PasteAttribsToSelSet(true);
						return TRUE;
					}
					case IDC_COPY_BUTTON:
					{
						theMapAttributes.CopySelection();
						theMapAttributes.UpdateCopyPasteButtons();
						return TRUE;
					}
					case IDC_BROWSE:
					{
						if (theMapAttributes.GetConfigFilesManager().EditConfigFileList(theMapAttributes.hDlg))
						{
							::SendMessage(theMapAttributes.m_hConfigList,CB_RESETCONTENT,0,0L);
							ConfigFilesManager &configFilesManager = theMapAttributes.GetConfigFilesManager();
							int numConfigFiles = configFilesManager.GetNumConfigFiles();
							...
							theMapAttributes.ChangeConfigFile(configFilesManager.GetCurrentConfigIndex());
						}
						return TRUE;
					}
				}
			...
			else if (HIWORD(wParam)==CBN_SELCHANGE)
			{
				...
				{
					int comboBoxSelection = ::SendMessage(theMapAttributes.m_hConfigList,CB_GETCURSEL,0,0L);
					int configSelection = ::SendMessage(theMapAttributes.m_hConfigList,CB_GETITEMDATA,comboBoxSelection,0L);
					theMapAttributes.ChangeConfigFile(configSelection);
				}
			}
			break;
		}
		...
		case WM_SETCURSOR:
		{
			if (NULL != theMapAttributes.m_hToolTip)
			{
				WORD message = HIWORD(lParam);
...
//------------------------------------------------------------------------
// --               MapAttributes::Member Functions                 --
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
void MapAttributes::SetupAttributes() 
// this method initializes the necessary data in order to support object
// attributes
{
	if (!m_pController)
	{
		m_pController = new MapAttrController();
	}
	...
	if (!hDlg)
	{
		...
		hDlg = CreateDialogParam(hInstance, MAKEINTRESOURCE(IDD_MAPATTRIB),
...
//------------------------------------------------------------------------
bool MapAttributes::AdjustPanelHeights(bool bUseDefaultSize)
...
// m_pUIController has been created in the MapAttrController object, and after
...
//
bool MapAttributes::SetUpUIDisplay(HWND hParentDlg, HINSTANCE hInstance,
...
//------------------------------------------------------------------------
bool MapAttributes::HandleWinMessage(HWND hParent, UINT message, WPARAM wParam, LPARAM lParam)

// This method will take the incoming message and pass it onto the
// MapAttrController to see if any of its controls want to handle the message.
// The return value from MapAttrController is then passed back as the return
...
//
bool MapAttributes::DrawUIDisplayLines(HDC hdc)
{
	enum DrawStyles
	{
		STYLE_BLANK,
...
//------------------------------------------------------------------------
bool MapAttributes::LoadCurrAttribData()
// MapAttributes::LoadCurrAttribData
...
bool MapAttributes::OkToCopy(void) const
{
	if (m_pController != NULL)
	{
...
bool MapAttributes::OkToPaste(void) const
{
	if (m_pController != NULL)
	{
...
bool MapAttributes::CopySelection(void)
{
	if (m_pController != NULL)
	{
...
//-----------------------------------------------------------------------------
// MapAttributes::PasteAttribsToSelSet
...
//		   NULL.
//-----------------------------------------------------------------------------
bool MapAttributes::PasteAttribsToSelSet(bool bReplace)
{
	bool bResult = false;
...
//------------------------------------------------------------------------
void MapAttributes::DestroyCurrAttribData()
...
//------------------------------------------------------------------------
void MapAttributes::DestroyUIDisplay()
...
//-----------------------------------------------------------------------------
void MapAttributes::SaveNonexclusiveKeys()
...
// Output: None.
//-----------------------------------------------------------------------------
void MapAttributes::InitKeyValPanel(ICustomRollupWindow* pCustomRollupWindow, HINSTANCE hInstance, LPARAM lParam)
{
...
// Output: None.
//-----------------------------------------------------------------------------
void MapAttributes::IdCancelMessage()
{
	m_pController->IdCancelMessage();
}

void MapAttributes::UpdateCopyPasteButtons(void)
{
	if (OkToCopy())
	{
...
//-----------------------------------------------------------------------------
// MapAttributes::SetApplyBttnStates
...
// Output: None.
//-----------------------------------------------------------------------------
void MapAttributes::SetApplyBttnStates(bool bApplySelEnabled, bool bApplySceneEnabled)
{
...
//-----------------------------------------------------------------------------
// MapAttributes::LoadKeyValPanelTree
...
// Output: None.
//-----------------------------------------------------------------------------
void MapAttributes::LoadKeyValPanelTree()
{
	m_pController->LoadKeyValPanelTree();
}

ConfigFilesManager &MapAttributes::GetConfigFilesManager(void)
{
	return *m_pConfigFilesManager;
}

void MapAttributes::ChangeConfigFile(int selectionIndex)
{
	m_pConfigFilesManager->SetCurrentConfigIndex(selectionIndex);
	NotifySelSetChanged(NULL);
}

void MapAttributes::RefreshConfig(void)
{
	if (m_pConfigFilesManager->NeedToLoadData())
	{
		m_pController->SaveNonexclusiveKeys();
