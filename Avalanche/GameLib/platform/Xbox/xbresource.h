...
public:
    ...
    HRESULT Create( const CHAR* strFilename, DWORD dwNumResourceTags = 0L, 
	 ...
	 HRESULT CreateDXD(DWORD Type, DWORD dwNumResourceTags, char* Fname  );


    HRESULT CreateFromHOG( const CHAR* strFilename, DWORD dwNumResourceTags = 0L, 
									XBRESOURCE* pResourceTags = NULL );
    
    HRESULT CreateFromMemory( char* Buffer, DWORD dwNumResourceTags = 0L,
                                 XBRESOURCE* pResourceTags = NULL);

	HRESULT AllocateMemory( int SysmemSize, int VidmemSize, char* Sysmem );
	HRESULT AllocateSystemMemory( int SysmemSize );
	HRESULT AllocateD3DMemory( int VidmemSize );
	HRESULT LoadDXData( const char* Fname, int DXDSize, BYTE* Buff );
	HRESULT CopySysMem (char* Sysmem);
	HRESULT SetVidMemPtr (BYTE* Ptr);
	BYTE*	  GetVidMemPtr (void);

    VOID Destroy();

 	 const char *	GetResourceFilename( void ) const { return m_pResourceFilename; }
    ...
    LPDIRECT3DRESOURCE8 RegisterResource( LPDIRECT3DRESOURCE8 pResource ) const
    {
        ...
        if( pResource && ( pResource->Common & D3DCOMMON_REFCOUNT_MASK ) == 1 )
        {
            ...
            if( ( pResource->Common & D3DCOMMON_TYPE_PUSHBUFFER ) &&
				...
				else if( pResource->Common & D3DCOMMON_TYPE_INDEXBUFFER )
					pResource->Data += (DWORD) m_pVidMemData;
            else
...
//    { return &m_pSysMemData[dwOffset]; }
    ...
    { return (LPDIRECT3DVERTEXBUFFER8)GetResource( dwOffset ); }
     ...
     LPDIRECT3DINDEXBUFFER8 GetIndexBuffer( DWORD dwOffset ) const
    { return (LPDIRECT3DINDEXBUFFER8)GetResource( dwOffset ); }
