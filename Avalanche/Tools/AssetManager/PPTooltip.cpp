		...
		else
		{
			...
			if (nIndexTool == m_nIndexDisplayWnd)
			{
				if (IsVisible())
				{
					...
					if (IsCursorInToolTip() && !(m_pToolInfo.nBehaviour & PPTOOLTIP_NOCLOSE_OVER))
					{
						Pop();
						m_nIndexCurrentWnd = PPTOOLTIP_TOOL_NOEXIST;
						m_nIndexDisplayWnd = PPTOOLTIP_TOOL_NOEXIST;
...
//	TRACE (_T("CPPToolTip::SetNewToolTip(Index = 0x%X)\n"), nIndexTool);
	...
	if (bWithDelay)
		...
		SetTimer(PPTOOLTIP_SHOW, m_nTimeReShow, NULL);
}
...
void CPPToolTip::CalculateInfoBoxRect(CPoint * pt, CRect * rect)
{
	...
	CRect rWindow(monitorInfo.rcWork);
...
//	HBITMAP hBitmap = m_pToolInfo.hBitmap;
	...
	CString strRes = _T("");  // The resource's name
			...
			case STATE_WAIT_TEXT_TAG:
				...
				case _T(' '):
					...
					else if (!strTag.CompareNoCase(_T("ct")))
					{
						//Color of the text
						nCmd = CMD_COLOR_TEXT;
						nParam = (UINT)m_crColor[PPTOOLTIP_COLOR_FG];
						nState = STATE_WAIT_BEGIN_NUMBER;
					}
					...
					else if (!strTag.CompareNoCase(_T("cb")))
					{
						...
						nCmd = CMD_COLOR_BK;
						nParam = (UINT)m_crColor[PPTOOLTIP_COLOR_BK_BEGIN];
						nState = STATE_WAIT_BEGIN_NUMBER;
					}
					else if (!strTag.CompareNoCase(_T("cbi")))
					{
						...
						nParam = PPTOOLTIP_COLOR_BK_BEGIN;
						nState = STATE_WAIT_BEGIN_NUMBER;
					}
		...
		if ((nState == STATE_WAIT_BEGIN_TAG) && (nCmd != CMD_NONE))
		{
			...
			case CMD_DRAW_BITMAP:
				if (!strRes.IsEmpty())
				{
					...
					if (bCalculate)
					{
						szLine = DrawResource(strRes, pDC, ptCur, 0, bCalculate);
						nLineHeight = max (nLineHeight, szLine.cy);
					}
					else
						szLine = DrawResource(strRes, pDC, ptCur, m_nHeightLines.GetAt(nLine), bCalculate);
...
CSize CPPToolTip::DrawResource(CString sName, CDC * pDC, CPoint pt, int nMaxHeight, BOOL bCalculate)
{
	CSize sz(0, 0);
	...
	{
	case TYPE_RES_ICON:
		sz = DrawIcon(pDC, pt, nMaxHeight, nr.nID, bCalculate);
		break;
	case TYPE_RES_BITMAP:
		sz = DrawBitmap(pDC, pt, nMaxHeight, nr.nID, FALSE, nr.crMask, bCalculate);
		break;
	case TYPE_RES_MASK_BITMAP:
		sz = DrawBitmap(pDC, pt, nMaxHeight, nr.nID, TRUE, nr.crMask, bCalculate);
		break;
	}
...
CSize CPPToolTip::DrawBitmap(CDC * pDC, CPoint pt, int nMaxHeight, UINT nID, BOOL bUseMask, COLORREF crMask, BOOL bCalculate)
{
	CSize sz(0, 0);
	...
	{
		CImageList img;
		img.Create(sz.cx, sz.cy, ILC_COLOR32 | ILC_MASK, 1, 1);
		img.Add(CBitmap::FromHandle(hBitmap), crMask);
		DrawIconFromImageList(pDC, pt, sz, img, 0, FALSE);
	}
	else
	{
		CDC memDC;
...
CSize CPPToolTip::DrawIcon(CDC * pDC, CPoint pt, int nMaxHeight, UINT nID, BOOL bCalculate)
{
	CSize sz (0, 0);
...
{
	if (m_imgTooltip.GetSafeHandle() == NULL)
		return CSize (0, 0);
...
HICON CPPToolTip::GetIconFromResources(UINT nID) const
{
...
void CPPToolTip::Activate(BOOL bActive)
{
	if (bActive)
	{
		m_bActive = TRUE;
...
static HWND foundChildScrollBarHWnd;
...
//		TRACE("Found ToolTip %08x\n",hwndChild);
	...
	if (myRect.PtInRect(mousePoint))
	{
		if (GetParent(hwndChild)!=parentHWND) // if a sub child
		{
			char className[80];
			GetClassName(hwndChild,className,80);
			if (!stricmp(className,"ScrollBar"))
			{
				foundChildScrollBarHWnd = hwndChild;
			}
			return TRUE; 
		}
		else
...
//	TRACE("Checking Point %d,%d\n",point.x,point.y);
	mousePoint = point;
	tooltipHWND = m_hWnd;
	parentHWND = m_pParentWnd->m_hWnd;
	testFlags = CWP_ALL;
	if (!bGetDisabled)
		testFlags |= CWP_SKIPDISABLED;
	foundChildHWnd = NULL;
	foundChildScrollBarHWnd = NULL;
	EnumChildWindows(parentHWND,EnumChildProc,0);
	if (foundChildScrollBarHWnd)
	{
		return NULL;
	}
	else 
...
//
/////////////////////////////////////////////////////////////////////////////
void CPPToolTip::AddTool(CWnd * pWnd, UINT nIdText, UINT nIdIcon,
						 LPCRECT lpRectTool /* = NULL */, UINT nIDTool /* = 0 */)
{
	CString str;
    str.LoadString(nIdText);
	AddTool(pWnd, str, nIdIcon, lpRectTool, nIDTool);
} // End AddTool
...
//
/////////////////////////////////////////////////////////////////////////////
void CPPToolTip::AddTool(CWnd * pWnd, CString sTooltipText, UINT nIdIcon,
						 LPCRECT lpRectTool /* = NULL */, UINT nIDTool /* = 0 */)
{
    ...
    { 
		...
		{ 
			...
			::ScreenToClient(hWnd, &ptClient); 
  			if (pToolInfo.rectBounds.PtInRect(ptClient)) 
			{
				return i;  
			}
		}
