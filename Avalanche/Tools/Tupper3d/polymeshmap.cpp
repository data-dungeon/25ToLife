...
// PolyMeshMap
//
// Class to handle polygon meshmaps
//
////////////////////////////////////////////////////////////////////////////
...
char *TupPolyMeshMap::m_keyNames[TupPolyMeshMap::NUM_CHILDREN_ATOMS] = 
{
	"MapName", // MAP_NAME
	"MapFlags", // MAP_FLAGS
	"MapVerts", // MAP_VERTS
...
TupPolyMeshMap::TupPolyMeshMap(TupperwareAggregate *pObject)
{
	m_pObject = pObject;
	BuildAtomCache();
}

void TupPolyMeshMap::BuildAtomCache(void)
{
...
TupperwareList *TupPolyMeshMap::AddFloatList(const float *pFloats,int numFloats,TupPolyMeshMap::ChildrenAtoms index)
{
...
TupperwareList *TupPolyMeshMap::AddIntList(const int *pInts,int numInts,TupPolyMeshMap::ChildrenAtoms index)
{
	TupperwareAtom *pAtom = m_cachedAtoms[index];
	// delete if there
	if (pAtom)
...
TupperwareScalar *TupPolyMeshMap::AddMapName(const char *name)
{
...
TupperwareScalar *TupPolyMeshMap::AddMapFlags(int mapFlags)
{
...
TupperwareList *TupPolyMeshMap::AddMapVerts(const float *pVerts,int numVerts)
{
	return AddFloatList(pVerts,numVerts*3,TupPolyMeshMap::MAP_VERTS);
}

TupperwareList *TupPolyMeshMap::AddMapIndices(const int *pIndices,int numIndices)
{
	return AddIntList(pIndices,numIndices,TupPolyMeshMap::MAP_INDICES);
}

TupperwareScalar *TupPolyMeshMap::GetMapName(char **name)
{
	TupperwareAtom *pAtom = m_cachedAtoms[MAP_NAME];
...
int TupPolyMeshMap::GetMapChannel(void)
{
...
TupperwareScalar *TupPolyMeshMap::GetMapFlags(int &mapFlags)
{
	TupperwareAtom *pAtom = m_cachedAtoms[MAP_FLAGS];
	if (pAtom && pAtom->GetAtomType()==TupperwareAtom::ATOM_SCALAR)
	{
...
TupperwareList *TupPolyMeshMap::GetFloatList(float **pFloats,int &numFloats,TupPolyMeshMap::ChildrenAtoms index)
{
	TupperwareAtom *pAtom = m_cachedAtoms[index];
...
TupperwareList *TupPolyMeshMap::GetIntList(int **pInts,int &numInts,TupPolyMeshMap::ChildrenAtoms index)
{
	TupperwareAtom *pAtom = m_cachedAtoms[index];
	if (pAtom && pAtom->GetAtomType()==TupperwareAtom::ATOM_LIST)
	{
...
TupperwareList *TupPolyMeshMap::GetMapVerts(float **pVerts,int &numVerts)
{
	TupperwareList *list = GetFloatList(pVerts,numVerts,TupPolyMeshMap::MAP_VERTS);
	// need to divide since there are 3 entrys per vertex
	numVerts/=3;
	return list;
}

TupperwareList *TupPolyMeshMap::GetMapIndices(int **pVerts,int &numIndices)
{
	return GetIntList(pVerts,numIndices,TupPolyMeshMap::MAP_INDICES);
}
