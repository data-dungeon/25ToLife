 ...
 * 
 * *****************  Version 24  *****************
 * User: Adam Clayton Date: 10/02/02   Time: 11:26a
 * Updated in $/Avalanche/tools/TupperFilters/ExportPS2
 * added uv limits for tristripper
 * 
 * *****************  Version 23  *****************
 ...
 * 
 * *****************  Version 21  *****************
 * User: Adam Clayton Date: 9/17/02    Time: 12:58p
 * Updated in $/Avalanche/tools/TupperFilters/ExportPS2
...
ExportPS2Filter::ExportPS2Filter(TupperChainLogBase &log) : FilterPlugin(log)
{
}

int ExportPS2Filter::GetTotalPasses(void) const
{
	return 1;
}

void ExportPS2Filter::Process(int pass,const FilterOptions &options,FilterDataManager &dataManager)
{
	...
	{
		throw(PluginException(ERROR_FATAL,"ExportPS2 Filter - Export\r\n  %s",(const char *)(e)));
	}
}

void ExportPS2Filter::GetFileDependencies(int pass,const FilterOptions &options,FilterDataManager &dataManager,FilePathArray &filePathArray)
{
	PluginLog::SetLog(&m_log);
	CheckOptions(options);
	TupString outputFilename(dataManager.GetOutputFilename());
...
void ExportPS2Filter::CheckOptions(const FilterOptions &options)
{
}

char *ExportPS2Filter::m_readDependencyNames[NUM_READ_DEPENDENCIES] = 
{
		...
		"TriStrip_MaxStripPixels = 2048", // MAX_STRIP_PIXELS
		"TriStrip_StripStageFaces = false", // STRIP_STAGE_FACES
		"MultiTex_NextStageFaceIndices", // NEXT_STAGE_FACE_INDICES
		
};

Revision ExportPS2Filter::m_readDependencyRevisions[NUM_READ_DEPENDENCIES] = 
{
...
int ExportPS2Filter::GetTotalReadDependencies(int pass,const FilterOptions &options) const
{
	return NUM_READ_DEPENDENCIES;
}

const char *ExportPS2Filter::GetReadDependencyKey(int pass,const FilterOptions &options,int index) const
{
	assert(index>=0 && index<NUM_READ_DEPENDENCIES);
	return m_readDependencyNames[index];
}

Revision ExportPS2Filter::GetReadDependencyRevision(int pass,const FilterOptions &options,int index) const
{
	assert(index>=0 && index<NUM_READ_DEPENDENCIES);
