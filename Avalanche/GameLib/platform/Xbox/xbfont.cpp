...
HRESULT CXBFont::Create( const CHAR* strFontResourceFileName, DWORD dwResourceOffset )
{
    ...
    if( FAILED( m_xprResource.Create( strFontResourceFileName, 0, NULL ) ) )
        return E_FAIL;
...
HRESULT CXBFont::CreateFromMemory( const CHAR* Buffer, DWORD dwResourceOffset )
{
    // Load the resource for the font
    if( FAILED( m_xprResource.CreateFromMemory((char*) Buffer ) ) )
        return E_FAIL;
...
//$TODO - remove this old way of doing fonts
...
//$END_TODO - remove this old way of doing fonts
    }
...
{
	m_fCursorX = Math::Floor( fCursorX );
	m_fCursorY = Math::Floor( fCursorY );
}
...
HRESULT CXBFont::Begin()
{
    ...
    if( 0 == m_dwNestedBeginCount )
    {
		...
		D3DDevice::SetRenderState (D3DRS_YUVENABLE, FALSE );   //pab added
                           ...
                           FLOAT fMaxPixelWidth )
{
    ...
    // Set the starting screen position
	 m_fCursorX = Math::Floor( fOriginX );
	 m_fCursorY = Math::Floor( fOriginY );
    ...
    if( dwFlags & XBFONT_CENTER_Y )
    {
        ...
        GetTextExtent( strText, &w, &h );
		  m_fCursorY = Math::Floor( m_fCursorY - h/2 );
    }
        ...
        if( bStartingNewLine )
        {
            if( dwFlags & (XBFONT_RIGHT|XBFONT_CENTER_X) )
            {
                ...
                if( dwFlags & XBFONT_RIGHT )
						 m_fCursorX = Math::Floor( fOriginX - w );
                if( dwFlags & XBFONT_CENTER_X )
						 m_fCursorX = Math::Floor( fOriginX - w/2 );
            }
