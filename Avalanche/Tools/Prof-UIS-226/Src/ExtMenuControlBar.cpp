...
HWND hWndMdiArea = NULL;
	if( pMdiFrame == NULL )
		hWndMdiArea = _GetHwndMdiArea();
	else
	{
		ASSERT_VALID( pMdiFrame );
		hWndMdiArea = pMdiFrame->m_hWndMDIClient;
...
HWND hWndActiveChildFrame = (HWND)
	...
	else
	{
		...
		DWORD dwActiveChildFrameStyle =
			::GetWindowLong( hWndActiveChildFrame, GWL_STYLE );
		if( pMdiFrame == NULL )
		{
...
bool CExtMenuControlBar::_InitMdiChildHook( HWND hWndMdiArea )
{
	ASSERT( m_bMdiApp );
	ASSERT( ::IsWindow(hWndMdiArea) );
	if( ! SetupHookWndSink( hWndMdiArea ) )
	{
		ASSERT( FALSE );
		return false;
	} // if( ! SetupHookWndSink( hWndMdiArea ) )
...
HWND hWndMdiArea = NULL;
	if( pMdiFrame == NULL )
		hWndMdiArea =
			((CExtMenuControlBar*)GetBar())->_GetHwndMdiArea();
	else
	{
		ASSERT_VALID( pMdiFrame );
		hWndMdiArea =
			pMdiFrame->m_hWndMDIClient;
	} // else from if( pMdiFrame == NULL )
	if( hWndMdiArea == NULL )
		return NULL;
	ASSERT( ::IsWindow( hWndMdiArea ) );
...
//		HWND hWnd = ((CExtMenuControlBar*)GetBar())->_GetHwndMainFrame();
//		if( hWnd == NULL )
//			return NULL;
//		ASSERT( ::IsWindow(hWnd) );
