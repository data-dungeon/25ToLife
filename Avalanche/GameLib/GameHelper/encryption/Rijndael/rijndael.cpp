...
int RijnDael_decrypt(char* InBuff, char* OutBuff, int Size, char* Password)
{
	...
	ASSERTS( (Size % 16) == 0, "Buffer size must be a multiple of 16");
	...
	while (1)
	{
		...
		memcpy (ciphertext, InBuff, 16);
		rijndaelDecrypt(rk, nrounds, ciphertext, plaintext);
		memcpy(OutBuff, plaintext, 16);
		InBuff += 16;
		OutBuff += 16;
		SizeCount += 16;
		if (SizeCount >= Size)
...
int RijnDael_encrypt( char* InBuff, char* OutBuff, int Size, char* Password)
{
	...
	while(1)
	{
		...
		unsigned char plaintext[16];
		unsigned char ciphertext[16];
		memcpy(plaintext, InBuff, 16);
		rijndaelEncrypt(rk, nrounds, plaintext, ciphertext);
		memcpy(OutBuff, ciphertext, 16);
		InBuff += 16;
		OutBuff += 16;
		SizeCount += 16;
		if (SizeCount >= Size)
