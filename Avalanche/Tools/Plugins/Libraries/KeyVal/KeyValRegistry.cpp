//-----------------------------------------------------------------------------
//	File:		KeyValRegistry.cpp
//	Created:	June. 21, 2002
//				Copyright (c) 2002, Avalanche Software Inc.
//				All Rights Reserved.
//	Author:		Adam Clayton
//
//	Desc:	The KeyValRegistry class handles storing, getting, setting, etc. of
...
//				6/21/2002: Copied this to the Keyval project - Brad Worthen
//-----------------------------------------------------------------------------
...
// Output: None.
//-----------------------------------------------------------------------------
KeyValRegistry::KeyValRegistry(char *keyName,HKEY hRootKey)
{
...
// Output: None.
//-----------------------------------------------------------------------------
KeyValRegistry::~KeyValRegistry(void)
{
	if (m_hKey)
	{
		RegFlushKey(m_hKey);
...
//-----------------------------------------------------------------------------
BOOL KeyValRegistry::QuerySize(const char *field,int *size)
{
...
//-----------------------------------------------------------------------------
BOOL KeyValRegistry::Set(const char *field,DWORD type,const void *data,int size)
{
	// Do we have an open entry?
	if (m_hKey == NULL)
		return(FALSE);
...
// Output: TRUE if successful, FALSE otherwise.
//-----------------------------------------------------------------------------
BOOL KeyValRegistry::Get(const char *field,DWORD type,void *data,int size)
{
	// Do we have an open entry?
	if (m_hKey == NULL)
		return(FALSE);
...
// Output: TRUE if successful, FALSE otherwise.
//-----------------------------------------------------------------------------
BOOL KeyValRegistry::Delete(const char *field)
{
	int tempSize; 
...
//-----------------------------------------------------------------------------
DWORD KeyValRegistry::EnumValue(DWORD dwIndex, LPTSTR lpValName, LPDWORD lpdValLength,
...
//-----------------------------------------------------------------------------
DWORD KeyValRegistry::GetNumValues()
{
	DWORD dwNumValues;
