...
eResizablePanelDockingType_t eResizablePanelDockingType =
		OnQueryDockingType();
	if( eResizablePanelDockingType == __RESIZABLE_DOCKING_TYPE_STUDIO_2005 )
		return true;
	return false;
}
...
eResizablePanelDockingType_t eResizablePanelDockingType =
		OnQueryDockingType();
	if(		bCancel
		&&	eResizablePanelDockingType == __RESIZABLE_DOCKING_TYPE_STUDIO_2005
		)
		bCancel = false;
...
#endif // (!defined __EXT_MFC_NO_TAB_CONTROLBARS )
				{
					CExtDynDockMarkerWnd::CreateDynamicMarkers( m_pDockSite, false );
					if( ! bForceFloatMode )
...
CExtControlBar::eResizablePanelDockingType_t eResizablePanelDockingType =
	...
	{
		if( eResizablePanelDockingType != CExtControlBar::__RESIZABLE_DOCKING_TYPE_STUDIO_2005 )
			return;
	}
	else
	{
...
bool bShowContent = m_pExtBarSrc->_IsShowContentWhenDragging();
	if(		bShowContent
		...
		&&	eResizablePanelDockingType != CExtControlBar::__RESIZABLE_DOCKING_TYPE_STUDIO_2005
		)
	{
		ASSERT_VALID( m_pExtBarSrc->m_pDockBar );
		CRect rcDockBar;
...
CFrameWnd * pParentFrame = GetParentFrame();
	if(		pParentFrame != NULL
		...
		&&	OnQueryDockingType() == __RESIZABLE_DOCKING_TYPE_STUDIO_2005
		)
		pParentFrame->ShowWindow( SW_HIDE );
...
CExtControlBar * pBar = _DraggingGetBar();
	if( pBar != NULL )
	{
		if( pBar->_IsShowContentWhenDragging() )
		{
			if(		(! pBar->IsFixedMode() )
				&&	(! pBar->IsFixedDockStyle() )
				&&	pBar->OnQueryDockingType() == __RESIZABLE_DOCKING_TYPE_STUDIO_2005
				)
			{
				POINT ptCursor;
				if( ::GetCursorPos( &ptCursor ) )
...
eResizablePanelDockingType_t eResizablePanelDockingType =
	...
	if( eResizablePanelDockingType == __RESIZABLE_DOCKING_TYPE_BY_THEME )
		eResizablePanelDockingType = (eResizablePanelDockingType_t)
			g_PaintManager->GetThemeDockingType();
...
HINSTANCE hInst = AfxGetInstanceHandle();
		...
		)
	{
		...
		wndclass.hbrBackground = NULL; 
		wndclass.lpszMenuName = NULL;
		wndclass.lpszClassName = __EXT_DYNDOCKHIGHLIGHTWND_CLASS_NAME;
		if( !::AfxRegisterClass( &wndclass ) )
		{
			ASSERT( FALSE );
			//AfxThrowResourceException();
...
CPaintDC dcPaint( this );
	if( rcClient.IsRectEmpty() )
		return;
	if( m_bmpHighlightSurface.GetSafeHandle() == NULL )
	{
		...
		m_bmpHighlightSurface.Attach( hDib );
		CBitmap * pOldBmp =
		...
		g_PaintManager->DockMarker_AdjustHighlightedArea(
			pSurfaceCB,
			bih.biWidth,
			bih.biHeight,
			bTabShape,
	...
	ASSERT( m_bmpHighlightSurface.GetSafeHandle() != NULL );
CDC dcMemory;
	if( dcMemory.CreateCompatibleDC( &dcPaint ) )
	{
		CBitmap * pOldBmp =
...
//	ASSERT( m_hWndMarkerTarget != NULL && ::IsWindow(m_hWndMarkerTarget) );
	...
	ASSERT( hBmpOut != NULL );
	ASSERT( hRgn != NULL );
	m_bmpIn.Attach( hBmpIn );
	m_bmpOut.Attach( hBmpOut );
	m_rgn.Attach( hRgn );
...
{
int nCount = arrDockMarkerWnds.GetSize();
...
{
	ShowMarkers( g_arrDockMarkerWndsOuter, bShow );
	ShowMarkers( g_arrDockMarkerWndsInner, bShow );
}

void CExtDynDockMarkerWnd::DestroyAllMarkers()
{
	DestroyMarkers( g_arrDockMarkerWndsOuter );
	DestroyMarkers( g_arrDockMarkerWndsInner );
	if( g_pWndMarkersParent != NULL )
	{
...
int nCount = arrDockMarkerWnds.GetSize();
	for( int nIndex = 0; nIndex < nCount; nIndex++ )
	{
		...
		} // else from if( (pDynDockMarkerWnd->GetStyle()&WS_VISIBLE) != 0 )
	} // for( int nIndex = 0; nIndex < nCount; nIndex++ )
	CExtPaintManager::stat_PassPaintMessages();
}
...
CRect rcScreen( layout.rect );
	...
	VERIFY(
		g_PaintManager->DockMarker_CreateWnds(
			NULL, //hWndLeftOver
	...
	if( ::GetCursorPos( &ptCursor ) )
	{
		...
		if( ! bFound )
		{
			...
			if( ! bFound )
			{
				...
				{
					//DWORD dwStyle = (DWORD)
					//	::GetWindowLong( hWndDynamicTarget, GWL_STYLE );
					//if( (dwStyle&WS_VISIBLE) == 0 )
					//	continue;
...
HINSTANCE hInst = AfxGetInstanceHandle();
		...
		)
	{
		...
		wndclass.hbrBackground = NULL; 
		wndclass.lpszMenuName = NULL;
		wndclass.lpszClassName = __EXT_DYNDOCKMARKER_CLASS_NAME;
		if( !::AfxRegisterClass( &wndclass ) )
		{
			ASSERT( FALSE );
			//AfxThrowResourceException();
...
void CExtDynDockMarkerWnd::OnTimer(UINT nIDEvent) 
{
	if( nIDEvent == 77  )
	{
		if( (GetStyle()&WS_VISIBLE) == 0 )
		...
		if( m_hWndMarkerTarget != NULL )
		{
			...
			{
				ShowMarkers( g_arrDockMarkerWndsInner, false );
				CExtControlBar * pDraggedBar =
					CExtControlBar::_DraggingGetBar();
				if( pDraggedBar == NULL )
					return;
