...
bool DBWriter::Attach( const char *pcFName, db_stream Type)
{
	...
	pStream = new std::ofstream( pcFName, std::ios::out | std::ios::binary);
...
//	pMyStream->Align( );
	...
	pMyStream->seekp( lFileHdrStart, std::ios::beg);
...
bool DBWriter::ChunkEnd( void)
{
	...
	std::ostream &str = pMyStream->stream( );

	lPos = str.tellp( );

	lChunkSize = lPos - lChunkStart;

	str.seekp( lChunkHdrStart, std::ios::beg);
	pMyStream->ResetAlignOffset( );	//consider ourselves aligned at this point
