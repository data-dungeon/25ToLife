...
inline void IntMap::GetKeyValue(const int idx,int &key,int &value)
{
	int at = 0;
	int count = -1;
...
inline bool IntMap::GetValue(const int key,int &value)
{
	if (m_capacity==0)
		return false;
...
inline int *IntMap::GetValuePtr(const int key)
{
	if (m_capacity==0)
		return false;
...
inline void IntMap::Grow(const int newcapacity)
{
	int at;
