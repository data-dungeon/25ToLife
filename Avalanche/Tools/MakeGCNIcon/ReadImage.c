...
BOOL ReadImage( HANDLE hFile, BITMAPINFO **bmi, char **bmBits)
{
	...
	switch( DetermineImageType( hFile, &sFrames))
	{
		case TYPE_BMP:
...
int DetermineImageType( HANDLE hFile, short *psFrames)
{
	
	char				header[sizeof(AnimHeaderDef)],
						footer[35];
	...
	bResult = ReadFile( hFile, header, sizeof(AnimHeaderDef), &nCountRead, NULL);
	if( !bResult)
	{
		return TYPE_UNKNOWN;
	}
	...
	*psFrames = 1;
	...
	{
		pAnimHdr = (AnimHeaderDef *)header;
		*psFrames = (short)pAnimHdr->nFrames;
		if (pAnimHdr->bHasLastDelta && pAnimHdr->bLastDeltaValid)
		{
			(*psFrames)--;
		}
...
{
	switch( DetermineImageType( hFile, nFrames))
	{
		case TYPE_ANM:
