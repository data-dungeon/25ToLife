...
CBaseTexture::CBaseTexture( CBundler * pBundler )
{
    ...
    m_dwLevels     = 1;
    m_pBundler     = pBundler;
    m_ColorKey     = 0; // color keying is effectively off, 
                        // since alpha must already be 0 to match
}
...
//-----------------------------------------------------------------------------
HRESULT CBaseTexture::LoadImage( char* pBuffer, int BufferSize, CImage** ppImage )
{
    CImage             ColorImage;
    CImage             AlphaImage;
    ...
    hr = ColorImage.Load( pBuffer, BufferSize );
    if( FAILED( hr ) )
        return hr;
    ...
    // Note that we need to specify dwWidth and dwHeight, since the whole point of
    // this resize is to get a common size, and because the loop below assumes
    // dwWidth and dwHeight. This was broken for a year and a half.
    pResizedColorImage = new CImage( dwWidth, dwHeight, D3DFMT_A8R8G8B8 );
    ...
    if( strAlphaSource[0] )
    {
        ...
        // this resize is to get a common size, and because the loop below assumes
        // dwWidth and dwHeight. This was broken for a year and a half.
        pResizedAlphaImage = new CImage( dwWidth, dwHeight, AlphaImage.m_Format );
                                            ...
                                            CImage** ppImage )
{
    ...
    if( FAILED( hr ) )
    {
        m_pBundler->ErrorMsg( "The bundler requires the D3D reference rasterizer in order "
                              "to load certain types of files, but was unable to create it. "
                              "Try using BMP or TGA files instead of DDS or JPG files.\n" );
        return hr;
    }
    ...
    hr = D3DXCreateTextureFromFileEx( m_pBundler->m_pd3dDevice, strImageFilePath, 
    ...
    if( m_nFormat < 0 )
    {
        ...
        if( m_nFormat < 0 )
        {
            m_pBundler->ErrorMsg( "Unsupported format for image <%s>\n", strSource );
            return E_FAIL;
        }
    ...
    // Do the size conversion for the source image
    hr = m_pBundler->m_pd3dDevice->CreateImageSurface( dwWidth, dwHeight, D3DFMT_A8R8G8B8,
