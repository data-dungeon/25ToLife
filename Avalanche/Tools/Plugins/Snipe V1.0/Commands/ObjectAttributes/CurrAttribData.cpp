...
//-----------------------------------------------------------------------------
// CurrAttribData::GetKeyValuePairs
// This method will return the number of attribs in this object.
//
// Input: None.
//
...
//-----------------------------------------------------------------------------
bool CurrAttribData::RemoveKeyname(const char *strKey)
{
	bool bResult = false;
...
//-----------------------------------------------------------------------------
bool CurrAttribData::RemoveAllKeynames(void)
{
	bool bResult = m_keyValuePairArray.GetSize()>0;
...
//
// Output: returns true if the array of key/value pairs were changed
//-----------------------------------------------------------------------------
bool CurrAttribData::AddKeyValue(const KeyValuePair &keyValuePair)
{
	int existingIndex = KeynameExistsAt(keyValuePair.m_strKey);
	// if key already exists
	if (existingIndex!=-1)
	{
		// if value for this key is the same
		if (m_keyValuePairArray[existingIndex] == keyValuePair)
		{
			// didn't need to add the key/value
			return false;
		}
		else
		{
			// change the value at this index
			m_keyValuePairArray[existingIndex].m_strValue = keyValuePair.m_strValue;
			return true;
		}
	}
	else
	{
		// key does not exist then add to end of array
...
// Output: returns true if the array of key/value pairs were changed
//-----------------------------------------------------------------------------
bool CurrAttribData::AddKeyValue(const char *pKey,const char *pValue)
{
	KeyValuePair newEntry(pKey,pValue);
	return AddKeyValue(newEntry);
}
...
// Output: returns true if the array of key/value pairs were changed
//-----------------------------------------------------------------------------
bool CurrAttribData::AddKeyValue(const char *pKey, bool bValue)
{
...
// Output: returns true if the array of key/value pairs were changed
//-----------------------------------------------------------------------------
bool CurrAttribData::AddKeyValue(const char *pKey, int nValue)
{
	KeyValuePair newEntry(pKey);
	char pChValue[50];
...
// Output: returns true if the array of key/value pairs were changed
//-----------------------------------------------------------------------------
bool CurrAttribData::AddKeyValue(const char *pKey, float fValue)
{
	KeyValuePair newEntry(pKey);
...
// Output: returns true if the array of key/value pairs were changed
//-----------------------------------------------------------------------------
bool CurrAttribData::AddKeyValue(const char *pKey,const AttribColor &colorValue)
{
	KeyValuePair newEntry(pKey);
...
void CurrAttribData::StrToCurrAttribData(const TupString &strData)
{
	...
	while (pos<strDataLength)
	{
		...
		if (equalsIndex != -1)
		{
			...
			// add to array (make sure there are no duplicate keys)
			AddKeyValue(newEntry); 
		}
		else
		{
...
//-----------------------------------------------------------------------------
// CurrAttribData::FormCheckListVal
// This method will form a checklist val based on the val already present on
// this object and the string and flags passed in.
//
// Input:
//  const TupString &strKeyname -- The keyname to use.
//  const TupString &strValue -- The "sub" value to form with.
//  const int bChecked -- true for checked, false for unchecked.
//
// Output: None.
//-----------------------------------------------------------------------------
void CurrAttribData::FormCheckListVal(const TupString &strKeyname, const TupString &strValue,  bool bChecked)
{
	TupString strFinalValue = "";
	TupString strCurrChecked = "";
	TupString strCurrUnchecked = "";

	// See if the keyname exists:
	int nKeyIndex = KeynameExistsAt(strKeyname);
	if (nKeyIndex > -1)
	{
		// Get the checked & unchecked substrings:
		TupString strCurrVal = GetValueAsString(nKeyIndex);
		int nDividerIndex = strCurrVal.Find(';');
		if (nDividerIndex < 0)
		{
			strCurrChecked = strCurrVal;
		}
		else
		{
			strCurrChecked = strCurrVal.Left(nDividerIndex);
			strCurrUnchecked = strCurrVal.Mid(nDividerIndex + 1);
		}
		
		// Now remove any occurance of the incoming val string from either
		// substring:
		int valAt = strCurrChecked.Find(strValue);
		while (valAt!=-1)
		{
			// Val string is in the strCurrChecked, but make sure it's not just
			// a substring of another name:
			bool bValNotSub = false;
			// Make sure next char is a comma:
			int commaIndex = strCurrChecked.Find(',', valAt);
			int nValLength = strValue.GetLength();
			if (commaIndex == (valAt + nValLength))
			{
				if (valAt != 0)
				{
					// Make sure prev char is either a comma or semicolon:
					if ((strCurrChecked[valAt - 1] == ',') || (strCurrChecked[valAt - 1] == ';'))
					{
						bValNotSub = true;
					}
				}
			...
			if (bValNotSub == true)
			{
				strCurrChecked.Delete(valAt, strValue.GetLength() + 1);
				break;
			}

			valAt = strCurrChecked.Find(strValue, valAt + 1);
		}
		valAt = strCurrUnchecked.Find(strValue);
		while (valAt!=-1)
		{
			// Val string is in the strCurrUnchecked, but make sure it's not just
			// a substring of another name:
			bool bValNotSub = false;
			// Make sure next char is a comma:
			int commaIndex = strCurrUnchecked.Find(',', valAt);
			int nValLength = strValue.GetLength();
			if (commaIndex == (valAt + nValLength))
			{
				if (valAt != 0)
				{
					// Make sure prev char is either a comma or semicolon:
					if ((strCurrUnchecked[valAt - 1] == ',') || (strCurrUnchecked[valAt - 1] == ';'))
					{
						bValNotSub = true;
					}
				}
			...
			if (bValNotSub == true)
			{
				strCurrUnchecked.Delete(valAt, strValue.GetLength() + 1);
				break;
			}

			valAt = strCurrUnchecked.Find(strValue, valAt + 1);
		}

		// Now form the strings:
		if (!bChecked)
		{
			// Put in unchecked string:
			strCurrUnchecked+=strValue;
			strCurrUnchecked+=",";
		}
		else
		{
			// Put in checked string:
			strCurrChecked+=strValue;
			strCurrChecked+=",";
		}
		strFinalValue=strCurrChecked;
		if (strCurrUnchecked.GetLength() > 0)
		{
			strFinalValue+=";";
			strFinalValue+=strCurrUnchecked;
		}
	}
	else
	{
	...
	RemoveKeyname(strKeyname);
	AddKeyValue(strKeyname, strFinalValue);
}
