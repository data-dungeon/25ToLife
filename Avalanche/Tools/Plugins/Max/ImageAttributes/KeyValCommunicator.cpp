...
// Output: None
//-----------------------------------------------------------------------------
KeyValCommunicator::KeyValCommunicator(ImgAttrController* pImgAttrController, char* pRegKeyName)
{
	m_pImgAttrController = pImgAttrController;
...
void KeyValCommunicator::LoadKeyValPanelData(Keeper<KeyInfo>& keyKeeper)
{
	...
	if (m_pImgAttrController != NULL)
	{
		// Tell the ImgAttrController to get the current attribute data:
		CurrAttribController* pCurrAttribController = m_pImgAttrController->GetCurrAttribController();
		if (pCurrAttribController != NULL)
		{
			...
			{
				CurrAttribData* pCurrAttribData = pCurrAttribController->GetObjectPtr(nIndex);
...
{
	// Tell the ImgAttrController to handle this:
	if (m_pImgAttrController != NULL)
		m_pImgAttrController->HandleApplySelect(keyKeeper, bApplyToAll);
}
...
void KeyValCommunicator::HandleApplyApply(Keeper<KeyInfo>& keyKeeper, bool bApplyToAll)
{
	// Tell the ImgAttrController to handle this:
	if (m_pImgAttrController != NULL)
		m_pImgAttrController->HandleApplyApply(keyKeeper, bApplyToAll);
}
...
void KeyValCommunicator::HandleApplyPurge(Keeper<KeyInfo>& keyKeeper, bool bApplyToAll)
{
	// Tell the ImgAttrController to handle this:
	if (m_pImgAttrController != NULL)
		m_pImgAttrController->HandleApplyPurge(keyKeeper, bApplyToAll);
}
...
void KeyValCommunicator::HandleApplyRemap(Keeper<KeyInfo>& keyKeeper, bool bApplyToAll)
{
	// Tell the ImgAttrController to handle this:
	if (m_pImgAttrController != NULL)
		m_pImgAttrController->HandleApplyRemap(keyKeeper, bApplyToAll);
}
