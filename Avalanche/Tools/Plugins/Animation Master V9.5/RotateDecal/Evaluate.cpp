...
BOOL CHxtApp::Evaluate(HTreeObject *htreeobject)
{
	...
	HashPatches selpatches,stamppatches;
...
Vector2 CHxtApp::RotateUV(const Vector2 uv,float angle,const Vector2 midpoint) {
	Matrix34 mat;
	mat.SetIdentity();
	mat.SetTranslate(Vector(-1*midpoint.x,-1*midpoint.y,0));
	Vector v(uv.x,uv.y,0);
	TransformVector(mat,v);

	Matrix34 rotmat = ZRotate(angle);
	TransformVector(rotmat,v);
...
void CHxtApp::RotateUVs(Vector2 *uvs,float angle,const Vector2 midpoint) {
	for (int i = 0; i < 12; i++)
		uvs[i] = RotateUV(uvs[i],angle,midpoint);
}
