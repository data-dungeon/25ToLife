...
void KeyValCommunicator::LoadKeyValPanelData(Keeper<KeyInfo>& keyKeeper)
{
	...
	if (m_pObjAttrController != NULL)
	{
		...
		{
			ConfigRoot *pConfigRoot = m_pObjAttrController->GetConfigRoot();
...
void KeyValCommunicator::AddKeyInfoData(CurrAttribController *pCurrAttribController,CurrAttribData* pCurrAttribData,Keeper<KeyInfo>& keyKeeper)
{
	...
	for (int contextKeyIndex = 0; contextKeyIndex < numContextKeys; contextKeyIndex++)
	{
		...
		if (pConfigBase)
		{
			if (pConfigBase->GetControlType() == ConfigBase::ControlType::CHECK_LIST)
			{
				...
				m_pObjAttrController->GetCheckListBoxValues(contextValueString,contextSubValues);
				int subIndex;
				...
				if (mountedKeyValueIndex != -1)
				{
					...
					m_pObjAttrController->GetCheckListBoxValues(mountedValueString,mountedSubValues);
	...
	for (keyIndex = 0; keyIndex < numMountedKeys; keyIndex++)
	{
		if (!usedMountedKeys[keyIndex])
		{
			...
			if (m_pObjAttrController->IsCheckListKey(newKeyInfo.m_strKeyname))
			{
				TupString valueString = keyValue.GetValueAsString(keyIndex);
				Keeper<TupString> subValues;
				m_pObjAttrController->GetCheckListBoxValues(valueString,subValues);
...
int KeyValCommunicator::HandleApplySelect(Keeper<KeyInfo>& keyKeeper, bool bApplyToAll)
{
	...
	if (m_pObjAttrController != NULL)
		totalSelected = m_pObjAttrController->HandleApplySelect(keyKeeper, bApplyToAll);
...
int KeyValCommunicator::HandleApplyApply(Keeper<KeyInfo>& keyKeeper, bool bApplyToAll)
{
	...
	// Tell the ObjAttrController to handle this:
	if (m_pObjAttrController != NULL)
		totalApplied = m_pObjAttrController->HandleApplyApply(keyKeeper, bApplyToAll);
	return totalApplied;
}
...
int KeyValCommunicator::HandleApplyPurge(Keeper<KeyInfo>& keyKeeper, bool bApplyToAll)
{
	...
	// Tell the ObjAttrController to handle this:
	if (m_pObjAttrController != NULL)
		totalPurged = m_pObjAttrController->HandleApplyPurge(keyKeeper, bApplyToAll);
	return totalPurged;
}
...
int KeyValCommunicator::HandleApplyRemap(Keeper<KeyInfo>& keyKeeper, bool bApplyToAll)
{
	...
	// Tell the ObjAttrController to handle this:
	if (m_pObjAttrController != NULL)
		totalRemapped = m_pObjAttrController->HandleApplyRemap(keyKeeper, bApplyToAll);
...
int KeyValCommunicator::HandleFind(const KeyValueFind &keyValueFind, bool bCurrentSelection, bool bAppendSelection)
{
	int totalFound = 0;
	if (m_pObjAttrController != NULL)
		totalFound = m_pObjAttrController->HandleFind(keyValueFind,bCurrentSelection,bAppendSelection);
	return totalFound;
}
...
{
	if (m_pObjAttrController != NULL)
		return m_pObjAttrController->IsCheckListKey(keyString);
	return false;
}
...
{
	if (m_pObjAttrController != NULL)
		return m_pObjAttrController->UpdateStatsText(statsLine);
