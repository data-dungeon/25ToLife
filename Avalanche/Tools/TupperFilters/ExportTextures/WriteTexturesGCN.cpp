...
void ProcessTup::WriteGCNTexture(TextureEntry &textureEntry,TupArray<TupArray<ts_TextureChunkRecord> > &textureChunkRecordArray,int frameNum,int mapIndex,bool bAnimation)
{
	...
	if (!bAnimation && numFrames>1)
	{
		...
		case TupImageDataFrame::INDEXED_4:
			int numAlphasUsed = textureEntry.m_pTextureAgg->FindScalarByKey("ExportTextures_PaletteAlphasUsed")->GetAsInt();
			if (numAlphasUsed>2)
				textureChunkRecord.u32Flags |= TEX_FLAG_SEPARATE_ALPHA_PLANE;
			break;
		}
	...
	case TupImageDataFrame::RGBA_1555:
		{
			...
			tupImageDataMap.GetData((void**)&pImageData,actualSize);
			assert(imageSize==actualSize);
			unsigned int	nBytesEncode = S3TC_GetEncodeSize( width, height, S3TC_ENCODE_RGB_ALPHA_COMPARE);
			S3TC_SetAlphaReference( 255);
			...
			s3texIn.pSurface = pImageData;
			s3texIn.PixelFormat.nARGBBitCount = 16;
			s3texIn.PixelFormat.nAlphaMask = 0x01 << 15;
			s3texIn.PixelFormat.nRedMask = 0x1F << 10;
			s3texIn.PixelFormat.nGreenMask = 0x1F << 5;
			s3texIn.PixelFormat.nBlueMask = 0x1F << 0;
			s3texIn.PixelFormat.nFlags = S3_TF_HASALPHA;
			pEncodedImage = new u8[nBytesEncode];
			int row,col;
			u8 lo,hi;
			int pixelOffset = 0;
			
			S3TC_Encode( &s3texIn, &s3texOut, pEncodedImage, S3TC_ENCODE_RGB_ALPHA_COMPARE, NULL, 0, 0);
			...
			else // first map
			{
				...
				else
				{
					...
					if (pParentPaletteReferenceScalar)
					{
						...
						assert(textureChunkRecord.u32PaletteOffset); // this better be set
						// set the separate alpha plane flag the same as my parent
						if (textureChunkRecordArray[parentPaletteReference][0].u32Flags & TEX_FLAG_SEPARATE_ALPHA_PLANE)
							textureChunkRecord.u32Flags |= TEX_FLAG_SEPARATE_ALPHA_PLANE;
					}
				}
			}
...
void ProcessTup::SetGCNTextureRecord(TextureEntry &textureEntry,TupArray<TupArray<ts_TextureChunkRecord> > &textureChunkRecordArray,int frameNum,int mapIndex,bool bAnimation)
{
	ts_TextureChunkRecord &textureChunkRecord = textureChunkRecordArray[frameNum][mapIndex];
