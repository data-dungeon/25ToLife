...
public:
	struct __PROF_UIS_API COLORREF_TABLE_ENTRY
	{
		...
		const UINT m_nStrTblResID;
		mutable CExtSafeString m_sColorNameLocal;
#endif // if (defined _AFXDLL && !defined __STATPROFUIS_WITH_DLLMFC__)
		const COLORREF m_clrValue;
		__EXT_MFC_SAFE_LPCTSTR m_sColorNameEnglish;
		COLORREF_TABLE_ENTRY(
			UINT nStrTblResID,
			COLORREF clrValue,
			__EXT_MFC_SAFE_LPCTSTR sColorNameEnglish
			);
		__EXT_MFC_SAFE_LPCTSTR GetLocalColorName() const;
	}; // struct COLORREF_TABLE_ENTRY

	static const COLORREF_TABLE_ENTRY g_colors[40];
