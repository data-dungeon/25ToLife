...
void StatsDisplayDialog::Update(void)
{
	...
	// Current instance
	SendDlgItemMessage(hDlg, ID_INSTANCENAME, WM_SETTEXT, 0, (LPARAM)stats.currentInstance.Data());

	sprintf(temp, "%d", stats.onInstance);
	SendDlgItemMessage(hDlg, ID_INSTANCE, WM_SETTEXT, 0, (LPARAM)temp);
...
LPARAM lParam)
{
	...
	switch (msg)
	{
		case WM_INITDIALOG:
			...
			// Static stuff
			SendDlgItemMessage(hDlg, ID_INPUTFILE, WM_SETTEXT, 0, (LPARAM)me->stats.inputFile);
			SendDlgItemMessage(hDlg, ID_OUTPUTFILE, WM_SETTEXT, 0, (LPARAM)me->stats.outputFile);
			sprintf(temp, "%d", me->stats.totalInstances);
