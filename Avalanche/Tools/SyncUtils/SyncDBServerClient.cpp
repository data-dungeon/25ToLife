...
SyncDBServerClient::Feedback::Feedback(LoadDBFeedback &loadDBFeedback) : m_loadDBFeedback(loadDBFeedback)
{
...
bool SyncDBServerClient::Feedback::Update(const char *pFileName)
{
...
SyncDBServerClient::SyncDBServerClient(const char *pFilePath,LoadDBFeedback &loadDBFeedback,LogFile &logFile) : 
...
SyncDBServerClient::~SyncDBServerClient()
{
	delete m_pTreeRoot;
	delete m_pGetCRC;
}

TreeNodeFolder *SyncDBServerClient::GetTreeRoot(void) const
{
	return m_pTreeRoot;
}

void SyncDBServerClient::SetupDatabase(LoadDBFeedback &loadDBFeedback)
{
	delete m_pTreeRoot;
	...
	if (!result)
	{
		// if we needed to stop then kiil the tree and all other stuff and return
		delete m_pTreeRoot;
		m_pTreeRoot = NULL;
		return;
	}
...
bool SyncDBServerClient::GetNodeChildren(TreeNodeFolder *pTreeNodeFolder,const char *pFilePathString,Feedback &feedback)
{
	CFileFind finder;
...
void SyncDBServerClient::SortChildren(TreeNodeFolder *pTreeNodeFolder,bool bSortChildFolders)
{
...
int SyncDBServerClient::CompareNodes(TreeNodeBase *pNode1,TreeNodeBase *pNode2)
{
...
TreeNodeFolder *SyncDBServerClient::CreateDirectory(SyncDBBase *pOtherDB,TreeNodeFolder *pOtherFolder,TreeNodeFolder *pParentFolder,SyncFeedback &feedback,bool bRight)
{
	BOOL bResult;
...
TreeNodeFile *SyncDBServerClient::CopyFileToDB(SyncDBBase *pOtherDB,TreeNodeFile *pOtherFile,TreeNodeFolder *pParentFolder,TreeNodeFile *pFile,SyncFeedback &feedback,int flags,bool bRight)
{
	BOOL bResult;
...
void SyncDBServerClient::GetPathToFile(const TreeNodeFile *pFile,TupString &filePath) const
{
	pFile->GetPath(filePath);
}

void SyncDBServerClient::AddChildNodeSorted(TreeNodeFolder *pParentFolder,TreeNodeBase *pNewChildNode)
{
...
bool SyncDBServerClient::DeleteFileFromDB(TreeNodeFile *pFile,SyncFeedback &feedback,bool bRight)
{
	BOOL bResult;
	// if file exists already in the database
	ASSERT(pFile);
...
bool SyncDBServerClient::DeleteDirectory(TreeNodeFolder *pFolder,SyncFeedback &feedback,bool bRight)
{
...
void SyncDBServerClient::CheckAndRemoveLog(const char *pPathString)
{
	CFileFind finder;
...
int SyncDBServerClient::RenameFolder(TreeNodeFolder *pFolder,const char *pNewName)
{
...
int SyncDBServerClient::RenameFile(TreeNodeFile *pFile,const char *pNewName)
{
...
bool SyncDBServerClient::CheckFileInSync(TreeNodeFile *pFile)
{
	TupString path;
	pFile->GetPath(path);
...
bool SyncDBServerClient::CheckAndTouch(SyncDBBase *pOtherDB,TreeNodeFile *pOtherFile,TreeNodeFile *pFile)
{
	FILETIME sourceFileTime = pOtherFile->GetModifiedTime();
	FILETIME destFileTime = pFile->GetModifiedTime();
...
// returns true if file time is updated
bool SyncDBServerClient::TouchFile(TreeNodeFile *pFile)
{
	FILETIME currentFileTime;
	SYSTEMTIME currentSystemTime;
...
bool SyncDBServerClient::GetCRCFromFile(TreeNodeFile *pFile,DWORD &crcValue) const
{
	if (pFile->ValidCRC())
	{
