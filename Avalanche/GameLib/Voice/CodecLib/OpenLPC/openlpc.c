...
#define FC		200.0f	/* Pitch analyzer filter cutoff */
#define DOWN		5	/* Decimation for pitch analyzer */
#define MINPIT		40.0f	/* Minimum pitch (observed: 74) */
#define MAXPIT		320.0f	/* Maximum pitch (observed: 250) */
...
static void calc_pitch(float w[], int len, float *per)
{
    ...
    *per = 0;	/* default: unvoiced */
    if ( x > MINPER &&  /* x could be < MINPER or even < 0 if pos == MINPER */
        ...
        vthresh = 0.6f; 
        if(r[0] > 0.002f)	   /* at low volumes (< 0.0f02), prefer unvoiced */ 
...
void init_openlpc_encoder_state(openlpc_encoder_state *st, int framelen)
{
    ...
    for (i = 0; i < st->buflen; i++) {
        st->s[i] = 0.0f;
        st->h[i] = (float)(WSCALE*(0.54f - 0.46f * cos(2.0f * M_PI * i / (st->buflen-1.0f))));
    }
    /* init the filters */
...
int openlpc_encode(const short *buf, unsigned char *parm, openlpc_encoder_state *st)
{
    ...
    for (i=0, j=st->buflen - st->framelen; i < st->framelen; i++, j++) {
        ...
        mkfilter -Bu -Hp -o 2 -a 0.0f125 -l -z */
        ...
        /* Digital filter designed by mkfilter/mkshape/gencode   A.J. Fisher
        MKFILTER.EXE -Bu -Lp -o 2 -a 0.0f375 -l -z */
...
int openlpc_decode(unsigned char *parm, short *buf, openlpc_decoder_state *st)
{
    ...
    gain = (float)((exp(gain) - 1.0f)/((2.718f-1.f)/10.0f));
#endif
    ...
    for (i=LPC_FILTORDER-1; i >= 0; i--) {
        ...
        k[i+1] = ((float)c / (1<<7));
#ifdef ARCSIN_Q
        if(i<2) k[i+1] = (float)sin(M_PI/2.0f*k[i+1]);
#endif
    }
