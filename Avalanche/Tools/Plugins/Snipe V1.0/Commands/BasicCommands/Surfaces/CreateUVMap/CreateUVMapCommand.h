...
class CreateUVMapCommand : public CommandInstance
{
public:
   enum Operation { OP_NEW, OP_REPLACE, OP_ADD, OP_SUBTRACT, OP_TOGGLE };

   String       m_strName;
   HUVMapSurfaceDriver *m_pUVMap;
   HUVMapSurfaceDriver::HDeleteInfo *m_pUVMapDeleteInfo;
   HProperty   *m_pSurfaceProperty;
   HImage      *m_pImage;
   CommandInstance *m_pSelectInstance;

            CreateUVMapCommand(HCommandPluginHelper *pPlugin);
   virtual ~CreateUVMapCommand();
...
class CreateUVMapCommandPlugin : public HCommandPluginHelper
{
public:
   DECLARE_COMMAND_PLUGINHELPER( CreateUVMapCommandPlugin, HCommandPluginHelper )
